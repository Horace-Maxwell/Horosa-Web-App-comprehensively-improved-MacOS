...
1. 后端主限法计算
2. `external-ref-Alchabitius` 与 `Horosa原方法` 双分支
...
- `external-ref_pd_virtual_body_corr_*.joblib`
...
- `/predict/pd` 可能走新实现
- 最终你会得到“广德盘第一页表格和 external-ref 差很大”的假象
...
- 不再报 `external-ref_alchabitius /chart and /predict/pd rows are not aligned`
...
python scripts/check_primary_direction_external-ref_integration.py
```
...
- `expected_results/runtime/pd_local/debug_guangde_exact_summary.json`
- `expected_results/runtime/pd_local/virtual_only_geo_current540_fullfit_summary.json`
- `expected_results/runtime/pd_local/stability_production_summary.json`
...
6. 广德盘浏览器专项通过
7. `external-ref-Alchabitius` 与 `Horosa原方法` 切换无串台

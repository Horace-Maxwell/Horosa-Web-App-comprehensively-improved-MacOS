# external-ref-Alchabitius 主限法复刻说明

本文档只描述 Horosa 当前生产版 `external-ref-Alchabitius` 分支，也就是 Horosa 网站里“主/界限法”页面顶部 `推运方法 = external-ref-Alchabitius` 时，真正落地运行的那一套主限法实现。

它不是文献抽象版，也不是早期实验版，而是“为了尽量原模原样复现 external-ref 当前网站输出”而落地到本地 Horosa 的工程实现说明。
...
- 相位：`0 / 60 / 90 / 120 / 180`
- `direct + converse`
- 当前 external-ref 网站真实支持的 shared-core 对象集
...
- `Mundo`
- 非 `Ptolemy` 时间键
- external-ref 不稳定或本地扩展的额外虚点
- `Terms / Bounds / PF / Dark Moon / Purple Clouds / South Node`

换句话说，本地是“精确复刻 external-ref 当前网站这条共享核心主限法链路”，不是重写一个理论上更纯的 Alchabitius 引擎。
...
- `pdMethod = external-ref_alchabitius`：走 external-ref 复刻内核
...
- 切换 `Horosa原方法 / external-ref-Alchabitius` 不是只改标题
...
- 这不是 external-ref 数学公式的一部分，但它是 Horosa 本地稳定复刻 `external-ref-Alchabitius` 主限法时必须保留的运行态约束；否则主限法页点击 `计算/重新计算` 会最先暴露成“本地排盘服务未就绪”。

## 4. 当前 shared-core 对象集

### 4.1 Promissor

当前 external-ref kernel 只计算：
...
当前 external-ref 选项下，不参与生产结果且前端会过滤的对象：
...
这是最关键的部分。  
如果只抄主公式，不抄下面这些工程层，本地结果不会接近 external-ref 当前网站。

### 5.1 对象集与相位集

固定对象集：

- `external-ref_PD_OBJECT_IDS = Sun..Pluto + North Node`
...
- `Horosa-Web/astropy/astrostudy/models/external-ref_pd_virtual_body_corr_sun_v1.joblib`
- `Horosa-Web/astropy/astrostudy/models/external-ref_pd_virtual_body_corr_moon_v1.joblib`
- `...`
- `Horosa-Web/astropy/astrostudy/models/external-ref_pd_virtual_body_corr_pluto_v1.joblib`
...
这层保证即使没有 body model，结果也仍然沿着 external-ref 当前口径收敛，而不是退回纯 Swiss 原始值。
...
- `_external-refAscCaseCorrection(chart)`

但当前生产逻辑是：

```python
external-ref_body_models_enabled = any(model_exists for Sun..Pluto)
...
生产版还必须包含这层显示过滤，否则表格不会像 external-ref。
...
也就是直接从 external-ref `sourceJD` 还原出 UTC 浮点小时，原样喂给 `PerChart`，避免字符串截断。
...
如果模型目录没带齐，本地仍能运行，但不会是“当前生产版 external-ref 近似口径”。
...
```bash
~/miniconda3/bin/python scripts/browser_horosa_master_check.py
```
...
- 易与三式全部子页
- `AI导出` / `AI导出设置`
- 主限法 `Horosa原方法 <-> external-ref-Alchabitius` 切换并重新计算
...
如果要“原模原样实现 Horosa 本地这套和 external-ref 非常接近的主限法”，不能只实现 Alchabitius 数学骨架，必须把下面这些一起做进去：
...
- `external-ref-Alchabitius` 这条当前生产链默认对应 `zodiaco主限法`
- 如果浏览器状态里残留了 `pdtype = 1`，那么页面标题虽然仍会显示 `external-ref-Alchabitius`，但后端实际会走 `mundo主限法`
- 这种偏差会让像广德盘 `2006-10-04 09:58 / 30n53 / 119e25` 这样的样本在第一页就明显偏离 external-ref
...
这和 external-ref 当前网页同一批广德样本的前几条已经重新对齐。
...
- `params.pdtype` 缺失
- `params.pdSyncRev` 缺失
- `params.pdSyncRev !== 'pd_method_sync_v6'`
...
- `pdTimeKey`
- `pdtype`
- `pdSyncRev = 'pd_method_sync_v6'`

并且它们和当前下拉选择完全一致时，主限法页面才允许进入 `已同步` 状态。

这样做的目的只有一个：

- 防止任何旧缓存、旧服务、旧返回、缺字段返回，在视觉上伪装成“当前 external-ref 主限法结果”。
...
- 浏览器第一页就会重新偏离 external-ref 当前网页结果。
...
- 桌面打包版里的浏览器页面
- 桌面打包版里的 Java `/chart`
- 桌面打包版里的 Python `external-ref-Alchabitius`

现在已经重新收敛到同一套生产结果。

## 17. `/chart` 与 `/predict/pd` 必须强制同源

这一轮整站自检里又暴露出一个非常关键、也非常容易误判为“主限法公式坏了”的问题：

- `/predict/pd` 明明已经按当前 `pdMethod` 正确返回；
- 但 `/chart` 仍可能带着旧的 `params.pdMethod`、旧的 `predictives.primaryDirection`；
- 浏览器表格是吃 `/chart` 的，因此用户会直接看到一套和 `/predict/pd` 不一致的主限法表。

具体现象是：

- 当切到 `Horosa原方法` 时，
  - `/predict/pd` 会返回 `1106` 行；
  - 但 `/chart` 仍可能回 `479` 行的 `external-ref-Alchabitius` 结果；
- 这样一来，用户会以为“主限法又和 external-ref 完全不同了”，其实是浏览器拿到了错误接口结果。
...
1. 把当前请求里的：
   ...
   - `pdSyncRev`
   强制写回到 `params`
2. 如果 `predictive=true`，则再直接调用：
   - `AstroHelper.getPrimaryDirection(args)`
3. 用返回的：
   - `pdres.get("pd")`
   覆盖 `predictives.primaryDirection`

这一步完成后，浏览器、`/chart`、`/predict/pd` 三者终于重新严格共用同一批 rows。

本轮修复后再跑运行态校验：

- `chart_external-ref_rows = 479`
- `chart_legacy_rows = 1106`
- `pd_external-ref_rows = 479`
- `pd_legacy_rows = 1106`
...
都已经在 `/chart` 与 `/predict/pd` 上重新对齐。

## 18. 一键部署必须能自愈损坏 venv

另一类问题不是主限法数学，而是包迁移后的运行环境损坏。

瘦身包、桌面包、GitHub 下载包经常会遇到：

- `.runtime/mac/venv/bin/pip` 的 shebang 仍指向旧绝对路径；
- 用户双击 `Horosa_OneClick_Mac.command` 时，刚好进入重建路径；
- 这时如果脚本仍直接调用 `bin/pip`，就会在安装依赖时失败。

因此当前生产版对一键部署也加了一条硬约束：

- 先检测 `.runtime/mac/venv` 是否真的可执行；
- 如果 venv 已损坏，整目录删除后重建；
- pip 调用统一使用：
  - `python -m pip`

这样做以后，迁移后的桌面包和 GitHub 下载包即使带着坏 shebang，也能在首启时自愈。

本轮已经实际验证：

- `HOROSA_NO_BROWSER=1 ./Horosa_OneClick_Mac.command`

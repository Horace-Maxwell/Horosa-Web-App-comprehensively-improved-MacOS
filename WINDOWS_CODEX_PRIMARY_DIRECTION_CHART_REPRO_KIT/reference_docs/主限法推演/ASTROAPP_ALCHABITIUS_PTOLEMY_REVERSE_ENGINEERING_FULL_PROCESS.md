# external-ref 的 Alchabitius + Ptolemy 主限法：Horosa 本地复刻的完整推理与实现过程

本文档记录的是一条完整的逆向与工程落地路径：

- 目标不是“讲解传统主限法理论”
- 而是“解释 Horosa 本地到底是如何一步步推理出 external-ref 当前网站的 `Alchabitius + Ptolemy` 主限法实现，并把它稳定落到本地生产代码里”
...
在 Horosa 本地复刻 external-ref 当前网站输出时，真正起作用的是 **多层叠加**：

1. 只对齐 external-ref 当前网站真实支持的 shared-core 对象集
2. 精确还原 external-ref 输入时间口径
...
- 数学骨架很重要
- 但工程细节同样重要
- 真正接近 external-ref 的不是“一个漂亮公式”，而是“公式 + 口径 + 修正层 + 接线”
...
- 只对齐 external-ref 当前网站真实输出的 shared-core rows
...
**external-ref 当前网站实际运行出来的那套结果。**

只要 external-ref 当前网站的实现与某些纯理论版本有偏差，我们就必须优先跟网站结果对齐。
...
- `external-ref_alchabitius`：external-ref 复刻方法
...
## 2.1 external-ref 主限法结果表

每个 case 目录里最关键的表是：

- `dirs.csv`

这张表提供 external-ref 当前网站输出的主限法行数据，至少包含：
...
## 2.2 external-ref chartSubmit 原始星体坐标

每个 case 目录里还有：

- `chartSubmit_response.xml`

这里给的是 external-ref 当前 chartSubmit 口径下的：
...
- 问题到底出在主限法公式
- 还是出在 external-ref 自己的“本体星历口径”与本地 Swiss 之间的微差
...
`sourceJD` 的意义尤其大，因为它让我们后面可以直接还原出 **external-ref 实际使用的 UTC 浮点小时**。

---

## 3. 我们是怎么把 external-ref 行和本地行一一对应起来的
...
- [scripts/compare_pd_backend_rows.py](~/Desktop/Horosa-Primary%20Direction%20Trial/scripts/compare_pd_backend_rows.py)
...
这不是理论约定，而是从 external-ref 输出行为里反推出来的兼容策略。
...
也就是只比较 external-ref 当前真实支持且映射稳定的那一组对象：
...
- 如果把 external-ref 不稳定映射的扩展虚点也混进来，误差会变成“比较对象错位”的误差
- 那不是算法误差，而是 scope 失真
...
- 传统公式方向大概率没错
- 但 external-ref 当前网站的坐标口径、对象集、角点处理、显示窗和本体星体坐标还有额外约束

换句话说：

**公式是骨架，但 external-ref 输出不是骨架本身。**
...
- external-ref 内部使用的是它真实计算时的 `sourceJD`
...
- [scripts/compare_pd_backend_rows.py](~/Desktop/Horosa-Primary%20Direction%20Trial/scripts/compare_pd_backend_rows.py)
- [scripts/check_primary_direction_external-ref_integration.py](~/Desktop/Horosa-Primary%20Direction%20Trial/scripts/check_primary_direction_external-ref_integration.py)
...
## 6. 第二层关键推理：external-ref 当前网站 shared-core 对象集比 Horosa 原始对象集更小

Horosa 原来能算的对象比 external-ref 当前网站多得多。

但我们真正要复刻的是 external-ref 网站，不是 Horosa 的扩展能力。

所以最终明确把 external-ref 分支缩到 shared-core：
...
- `Purple Clouds`
- `South Node`
- 其它 external-ref 当前不稳定或不支持的扩展虚点

这是一个非常重要的策略转折。

它的意义不是“偷懒”，而是：

- 只对齐 external-ref 当前可验证范围
- 先把可验证部分做到稳定
- 再谈其它对象
...
- [Horosa-Web/astropy/astrostudy/perpredict.py](~/Desktop/Horosa-Primary%20Direction%20Trial/Horosa-Web/astropy/astrostudy/perpredict.py)

里的：

- `_external-refTrueNodeBaseLons`
- `_rebuildexternal-refNodePoint`
...
- external-ref 当前网站结果更接近“按日期动态取黄赤交角”的口径
...
当 `Asc` 已经切到 OA 链路之后，误差虽然大幅下降，但仍然没有完全贴住 external-ref。
...
这个值不是传统文献给出来的，它是从当前 external-ref 网站输出行为里逆出来的工程常量。
...
## 11. 第七层关键推理：显示窗不是装饰，而是 external-ref 表格输出的一部分

如果你只做 arc 公式，而不做显示窗过滤，你仍然会发现：

- 本地表格和 external-ref 行数对不上
- 前导 rows 顺序和内容也不对

于是后面确认 external-ref 对普通 planet-pair 行还有一层显示窗行为。
...
这一层很容易被忽略，但实际上它是“为什么本地表格看起来像 external-ref”的一部分。
...
**问题不再主要是 arc 公式，而是 external-ref chartSubmit 本体坐标与本地 Swiss 本体坐标之间还差一点点。**
...
- [scripts/train_external-ref_virtual_body_corrections.py](~/Desktop/Horosa-Primary%20Direction%20Trial/scripts/train_external-ref_virtual_body_corrections.py)
...
- `external-ref_pd_asc_case_corr_et_v1.joblib`
...
- external-ref 网站自身口径在不同批次之间可能有变化
...
- 有一类整图偏差在这批样本上被放大
- 但当前 external-ref 网站口径并不更接近这批

后来的结论是：

- 不能让 `wide240_v7` 继续主导生产调参
- 当前 external-ref 网站更接近 `geo300 / run200_login`
...
1. external-ref 的 `chartSubmit_response.xml` 已经足够给出它当前网站的本体坐标层证据
2. 我们真正要对齐的是 external-ref 当前网站，不是通用天文软件显示
3. 后期残差主要来自 external-ref 本体与本地 Swiss 的微差，而不是“缺一个外部天文坐标参考”
...
- `getPrimaryDirectionByZexternal-refKernel()`
...
这就是当前 Horosa 本地 external-ref 分支真正运行的东西。
...
- 打开 `AI导出` 与 `AI导出设置`；
- 并真实切换 `Horosa原方法 <-> external-ref-Alchabitius` 后点击 `重新计算`。
...
- [Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js](~/Desktop/Horosa-Primary%20Direction%20Trial/Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js)
...
- [Horosa-Web/astrostudyui/src/components/direction/AstroDirectMain.js](~/Desktop/Horosa-Primary%20Direction%20Trial/Horosa-Web/astrostudyui/src/components/direction/AstroDirectMain.js)
...
- 页面标题已经是 `external-ref-Alchabitius`
...
- [Horosa-Web/astrostudyui/src/utils/aiExport.js](~/Desktop/Horosa-Primary%20Direction%20Trial/Horosa-Web/astrostudyui/src/utils/aiExport.js)
...
- 新旧分支都存在
- `external-ref-Alchabitius` 行不含前端隐藏对象
...
**不要把 external-ref 主限法当成一条待发现的纯数学公式，而要把它当成一个“带输入口径、对象范围、坐标变换、微调层、显示窗、排序、导出链路”的完整生产系统。**
...
- 能从 external-ref 当前网站输出中，逐层分离“理论骨架”和“实现口径”
- 知道哪些是传统结构
- 哪些是网站当前实现的工程特征
...
- `WINDOWS_CODEX_external-ref_PD_REPRO_KIT/`

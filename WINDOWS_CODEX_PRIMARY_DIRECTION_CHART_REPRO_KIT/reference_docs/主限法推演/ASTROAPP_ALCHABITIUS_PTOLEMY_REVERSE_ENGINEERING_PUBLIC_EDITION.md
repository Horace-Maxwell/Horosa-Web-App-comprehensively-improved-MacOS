# external-ref `Alchabitius + Ptolemy` 主限法本地复刻：一份适合公开发布的工程整理稿

## 摘要

本文讨论的不是“传统主限法理论如何讲解”，而是一个更具体、也更适合当代工程环境的问题：

**如何在本地系统中，尽可能稳定、可验证地复刻 external-ref 当前网站所输出的 `Alchabitius + Ptolemy` 主限法结果。**
...
- [perpredict.py](~/Desktop/Horosa-Primary%20Direction%20Trial/Horosa-Web/astropy/astrostudy/perpredict.py)
- [PRIMARY_DIRECTION_external-ref_ALCHABITIUS_REPLICATION.md](~/Desktop/Horosa-Primary%20Direction%20Trial/PRIMARY_DIRECTION_external-ref_ALCHABITIUS_REPLICATION.md)
- [external-ref_ALCHABITIUS_PTOLEMY_REVERSE_ENGINEERING_FULL_PROCESS.md](~/Desktop/Horosa-Primary%20Direction%20Trial/external-ref_ALCHABITIUS_PTOLEMY_REVERSE_ENGINEERING_FULL_PROCESS.md)

## 一、问题的定义：复刻“当前网站输出”，而不是抽象理论

必须先明确目标，否则整个工程会迅速发散。

本项目要复刻的，不是所有可能版本的 Alchabitius 主限法，也不是某一本古典教材中的理想形态，而是 external-ref 当前网站在以下条件下的实际行为：
...
这套复刻方法成立的前提，是直接使用 external-ref 的原始输出，而不是只看网页截图或人工记录。

实际使用的证据主要分为三层。

### 1. 主限法结果表

每个 case 目录中的 `dirs.csv` 记录了 external-ref 当前网站的主限法结果行。它至少提供：
...
- 主限法公式本身的误差
- external-ref 本体星历口径与本地 Swiss 之间的微差
...
其中 `sourceJD` 极其关键，因为它使得本地可以还原 external-ref 实际使用的 UTC 浮点时间，而不是只用字符串级别的出生时间近似代替。
...
- [compare_pd_backend_rows.py](~/Desktop/Horosa-Primary%20Direction%20Trial/scripts/compare_pd_backend_rows.py)
...
这不是理论规定，而是从 external-ref 当前输出行为中归纳出的兼容策略。

### 3. `shared-core-only`

最终生产比较只保留 external-ref 当前稳定支持且映射明确的 shared-core 对象集：
...
- external-ref 内部真正参与计算的是 `sourceJD`
...
## 五、第二条决定性推理：对象集必须以 external-ref 当前网站为准

Horosa 原有主限法能计算的对象比 external-ref 当前网站更多，但复刻目标不是 Horosa 的历史能力，而是 external-ref 的当前行为。

因此最终将 external-ref 分支限制在 shared-core 对象集内：
...
这是整个项目中证据最直接的一环。通过 external-ref 响应数据可确认：
...
后续比较逐步确认，external-ref 当前网站更接近按日期动态计算的黄赤交角：
...
这一点说明 external-ref 的当前实现并非简单依赖固定传统常量，而更接近实时天文坐标口径。
...
这一步与传统资料里“`MC` 看 RA、`Asc` 看 OA”的结构是吻合的，但工程实现更细，因为每一侧到底用真实黄纬还是 zero-lat，并不是文献直接给出的，而是通过 external-ref 输出行为逐步识别出来的。
...
如果只关心 arc 公式，而忽略 external-ref 的表格显示逻辑，那么最终即使 arc 很接近，页面行数和排序也会明显不同。
...
其本质不是“界面装饰”，而是 external-ref 表格生成规则的一部分。  
...
- external-ref 当前网站本体坐标与本地 Swiss 之间确实存在微差
- 这些微差会在虚点行上被放大
...
- [train_external-ref_virtual_body_corrections.py](~/Desktop/Horosa-Primary%20Direction%20Trial/scripts/train_external-ref_virtual_body_corrections.py)
...
- [perchart.py](~/Desktop/Horosa-Primary%20Direction%20Trial/Horosa-Web/astropy/astrostudy/perchart.py)
- [webchartsrv.py](~/Desktop/Horosa-Primary%20Direction%20Trial/Horosa-Web/astropy/websrv/webchartsrv.py)

### 3. 前端方法切换

- [AstroPrimaryDirection.js](~/Desktop/Horosa-Primary%20Direction%20Trial/Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js)
- [AstroDirectMain.js](~/Desktop/Horosa-Primary%20Direction%20Trial/Horosa-Web/astrostudyui/src/components/direction/AstroDirectMain.js)
...
- 点击左侧主模块和关键右栏技术页；
- 打开 `AI导出` 与 `AI导出设置`；
- 在主限法页真实切换 `Horosa原方法` 与 `external-ref-Alchabitius` 并重新计算。
...
- [aiExport.js](~/Desktop/Horosa-Primary%20Direction%20Trial/Horosa-Web/astrostudyui/src/utils/aiExport.js)
...
- [check_primary_direction_external-ref_integration.py](~/Desktop/Horosa-Primary%20Direction%20Trial/scripts/check_primary_direction_external-ref_integration.py)
...
- [verifyPrimaryDirectionRuntime.js](~/Desktop/Horosa-Primary%20Direction%20Trial/Horosa-Web/astrostudyui/scripts/verifyPrimaryDirectionRuntime.js)

### 4. 整站 smoke check

- [verifyHorosaRuntimeFull.js](~/Desktop/Horosa-Primary%20Direction%20Trial/Horosa-Web/astrostudyui/scripts/verifyHorosaRuntimeFull.js)
- [check_horosa_full_integration.py](~/Desktop/Horosa-Primary%20Direction%20Trial/scripts/check_horosa_full_integration.py)
...
**external-ref 的主限法不能被理解为一条孤立公式，它必须被理解为“输入口径 + 对象范围 + 坐标转换 + 本体修正 + 显示规则 + 前后端接线 + 验证体系”共同构成的生产系统。**

这也是为什么很多“我知道 Alchabitius 理论”的人，仍然无法稳定复刻 external-ref 当前网站输出。  
他们掌握的是骨架，但缺少系统。
...
- [PRIMARY_DIRECTION_external-ref_ALCHABITIUS_MATH_FLOW.md](~/Desktop/Horosa-Primary%20Direction%20Trial/PRIMARY_DIRECTION_external-ref_ALCHABITIUS_MATH_FLOW.md)
...
就这个项目而言，真正值得公开的，不只是 Horosa 现在已经很接近 external-ref 的结果，而是：

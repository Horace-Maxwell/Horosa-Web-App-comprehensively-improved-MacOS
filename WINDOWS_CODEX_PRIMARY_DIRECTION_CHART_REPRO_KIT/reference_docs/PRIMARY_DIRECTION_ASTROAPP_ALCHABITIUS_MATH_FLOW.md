# external-ref-Alchabitius 主限法数学版与流程图

这份文档描述 Horosa 当前生产版 `external-ref-Alchabitius` 的数学骨架，以及它在工程里额外叠加的修正层。
...
当前本地实现要逼近 external-ref，实际由两部分组成：

1. 主限法数学骨架
2. 为逼近 external-ref 当前网站输出而加上的工程修正
...
- 直接使用 external-ref `sourceJD`
- 还原精确 UTC 浮点小时
- 不做字符串时间截断

## 14. 当前生产版为何能接近 external-ref
...
flowchart TD
  ...
  B --> C["pdMethod = external-ref_alchabitius"]

...
- `推运盘 -> 主/界限法`
- `推运盘 -> 主限法盘`
- `external-ref-Alchabitius` 主限法算法
- `Horosa原方法` 与 `external-ref-Alchabitius` 的双分支切换
- `主限法盘` 的任意时间外圈投影
...
- 广德盘浏览器实测结果与 external-ref 当前输出近似对齐
...
2. `/chart` 与 `/predict/pd` 在 `external-ref_alchabitius` 分支上行级一致
...
6. 广德盘：
   ...
   - 经纬：`30N53 / 119E25`
   - 地点：`guangde`
   - 方法：`external-ref-Alchabitius + Ptolemy`
   - 浏览器第一页前几行应接近：
...
- `reference_docs/主限法推演/PRIMARY_DIRECTION_external-ref_ALCHABITIUS_REPLICATION.md`
- `reference_docs/主限法推演/PRIMARY_DIRECTION_external-ref_ALCHABITIUS_MATH_FLOW.md`

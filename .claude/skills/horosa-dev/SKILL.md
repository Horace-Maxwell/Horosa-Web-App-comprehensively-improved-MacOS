...
**强制，针对每一次改动 Horosa：**
1. **先读这两份文档**，再动手：本 skill（`.claude/skills/horosa-dev/实现说明`）+ 仓内 dev-docs 文档
   `实现说明`。两者是「会反复踩」的坑清单 + dev/release 流程；跳过 = 重付一轮 debug。
2. **解决任何新问题后立刻回写**：根因 + 修法写进 `实现说明`（机制坑）或本 skill（dev/发布坑），
...
6. **Umi builds are sequential.** Do not run `npm run build` and `npm run build:file` in parallel; both write
   ...
   and rerun the builds one at a time. Cache deletion is intentionally not auto-approved by the dev-docs; ask before
   using destructive cleanup commands.
7. **Claude dev-docs JSON is part of the project.** `settings.json`, `settings.local.json`, and `launch.json` must
...
# Focused automated tests for the current AI分析/kentang dev-docs surface
...
3. **dev-docs green:** JSON parse `.claude/settings.json`, `.claude/settings.local.json`, and `.claude/launch.json`;
...
2. Run the pre-release gates: dev-docs JSON, focused tests, clean sequential `npm run build` then `npm run build:file`,
...
- **Run `Horosa_Desktop_Installer/scripts/release_preflight.sh` before publishing** — `publish_github_release.sh`
  ...
  `.claude/settings.local.json` not git-tracked + all dev-docs JSON parseable, `astrostudyboot.jar`
...
**Self-improvement — do this EVERY release (the dev-docs must get more reliable, not drift):**

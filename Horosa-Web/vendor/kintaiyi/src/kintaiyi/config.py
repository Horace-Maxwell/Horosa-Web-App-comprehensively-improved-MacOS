...
def starhouse(year, month, day, hour, minute):
    """二十八宿值日:自当前节气的宿度锚点起,按【日行一度】推得当日太阳所值之宿。

    分度制＝古度(周天 365 度余),不是 360:本式把 distancejq 返回的【天数】不加换算
    直接累进到【度】索引上,唯有日行一度、周天 365 度余的古法框架才自洽;若按 360,
    一年会多走 5 度而永不闭合。numlist 即各宿距度(古度),下同。

    numlist 与《汉书·律历志》古度表逐位比对(角12 亢9 氐15 房5 心5 尾18 箕11 / 斗26
    牛8 女12 虚10 危17 室16 壁9 / 奎16 娄12 胃14 昴11 毕16 觜2 参9 / 井33 鬼4 柳15
    星7 张18 翼18 轸17,合 75+98+80+112=365 正合周天):28 项中 26 项差 ≤2(历代实测
    宿度本有此量级的出入),仅两项离群——虛 +15、井 -3。
    其中「虛=25」经两个独立史料源核对皆远小于此:《汉书·律历志》作 10,元《授时历》
    量天尺作 9(虚9°、危15.95°、室18.32°、壁9.34°、奎17.87°);且 25 与自身表内所有
    邻项量级不符,判为誊抄之误,今改回 10(全表合 378 → 363)。
    井 30 vs 33 差 3,仍在历代变体的散布内,不动。**不整表替换成汉书**:下方 jqsulist
    的节气宿度锚点与本表须同出一个历元,换表不换锚点反而会拆散两者的配对。
    """
    #                角  亢  氐 房 心  尾  箕  斗 牛  女  虛  危  室  壁  奎  婁  胃  昴  畢 觜 參  井 鬼  柳 星  張  翼  軫
    numlist = [13, 9, 16, 5, 5, 17, 10, 24, 7, 11, 10, 18, 17, 10, 17, 13, 14, 11, 16, 1, 9, 30, 3, 14, 7, 19, 19, 18]
    ...
    zhoutian = len(gensulist)   # 周天总度＝本表之和;旧代码写死 360,与表长不符即静默混叠
    # 二十四节气宿度锚点 [宿, 入宿度]。以真实节气间隔(14~16 日)逐个校核后,三处修正:
    #  · 霜降/立冬 原作 房/氐 —— 氐(环上第 2) 在 房(第 3) 之前,原序令太阳倒退一宿
    #    (全环唯一一处逆行),今换正为 氐/房。
    #  · 夏至 原作 井1 —— 与前锚 參6 只隔 4 度、与后锚 井27 隔 26 度,而全环其余各步皆 14~16。
    #    由前后两锚等分得 井12(參6→井12=15,井12→井27=15)。
    #  · 大雪 原作 箕24 —— 箕宿只有 10 度,该值已越出箕宿落进斗宿,致岁末太阳倒退。
    #    由前后两锚等分得 箕4(尾6→箕4=15,箕4→斗9=15)。「24」疑抄自紧邻的小寒「斗24」。
    # 其余各锚与实测间隔差 ≤3 度,属历元岁差与近日点快慢的正常散布,不动。
    # 🔴 判据用的是【相邻节气间隔】而非比对某朝史料——本表整体与汉书十二次有系统性
    #    偏移(不同历元/岁差所致,属正常),故不能逐条对照史料判对错;但无论哪个历元,
    #    24 个节气必等分周天,每步应 ≈363/24≈15.1 度。实测:修前 7 步畸形(4/26/30/30/35
    #    及两处负步即倒退),修后 24 步全落 13~17。这个判据不依赖历元,故最可靠。
    jqsulist = [["斗", 9],["斗",24] ,["女", 8],["危",2],["室", 1],["壁",1] ,["奎", 4],["婁",2] ,["胃", 4],["昴",4] ,["畢", 8],["參",6] ,["井", 12],["井", 27],["柳",8] ,["張", 3],["翼",1] ,["翼", 16],["軫",13] ,["角", 9],["氐", 2],["房",1] ,["尾", 6],["箕",4]]
    ...
    anchor = njq_list.get(currentjq)
    if not anchor:
        # 不可静默取首宿:那会把「节气名对不上」伪装成一个像模像样的宿名交出去。
        # jq() 与本表的名集实测全同(各 24 名),故此路只可能是上游改名/域外回退所致,须炸出来。
        raise KeyError(f"starhouse: 节气「{currentjq}」不在宿度锚点表内(应为 jieqi_name 的 24 名之一)")
    num = gensulist.index(anchor[0]) + anchor[1] + distance_to_cjq
    return gensulist[num % zhoutian]
#%% 太乙十精
#五行

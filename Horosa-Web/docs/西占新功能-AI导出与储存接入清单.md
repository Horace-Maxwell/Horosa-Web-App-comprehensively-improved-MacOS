...
| pdMethod 随命盘存档 | ❌（默认回退 astroapp_alchabitius）；**注意属 PD 核心，改动须守铁律** | ⏳ 待修(谨慎) |
...
- **验证**:npm build 绿;`aiAnalysisContext.test`+`aiAnalysisSelection.test` 等 29 套 140 测试全过(含技法 missing 契约)。界推运/Huber 的 AI分析挂载真机验受 antd Select 合成事件限制未驱动,但 fetch/格式与**用户本会话已对 astroapp 验过的同名组件完全一致**、且走既有 firdaria wired 模式。

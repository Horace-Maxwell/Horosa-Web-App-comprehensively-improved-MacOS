...
MAX_ERROR = 0.0003
external-ref_PD_DISPLAY_EPS = 3.0
external-ref_PD_DISPLAY_WINDOW = 107.5
external-ref_PD_ASC_PROM_TRUE_OBLIQUITY_OFFSET = -0.0014
external-ref_PD_PROM_CORR_JD_CENTER = 2460500.0
external-ref_PD_ASC_CASE_CORR_MODEL = (
    Path(__file__).resolve().parent / 'models' / 'external-ref_pd_asc_case_corr_et_v1.joblib'
)
external-ref_PD_VIRTUAL_BODY_CORR_MODEL_DIR = Path(__file__).resolve().parent / 'models'
external-ref_PD_VIRTUAL_BODY_CORR_MODELS = {
...
external-ref_PD_SIGNIFICATOR_IDS = [
    *external-ref_PD_PROMISSOR_IDS,
]
external-ref_PD_VIRTUAL_SIGNIFICATOR_IDS = {
    const.ASC,
    const.MC,
    const.PARS_FORTUNA,
    const.NORTH_NODE,
}
_external-ref_PD_ASC_CASE_CORR_MODEL_CACHE = None
_external-ref_PD_ASC_CASE_CORR_MODEL_READY = False
_external-ref_PD_VIRTUAL_BODY_CORR_MODEL_CACHE = {}
_external-ref_PD_VIRTUAL_BODY_CORR_MODEL_READY = set()
_external-ref_PD_VIRTUAL_BODY_CORR_DELTA_CACHE = {}
...
class PerPredict:
    ...
    def getPrimaryDirectionByZ(self):
        if getattr(self.perchart, 'pdMethod', 'external-ref_alchabitius') == 'horosa_legacy':
            pdlist = self.getPrimaryDirectionByZLegacy()
        else:
            pdlist = self.getPrimaryDirectionByZexternal-refKernel()
        self.appendDateStr(pdlist)
        return pdlist
    ...
    def _external-refMeanObliquity(self, chart):
        # external-ref's zodiacal PD rows align best when the ecliptic->equatorial
    ...
    def _external-refTrueObliquity(self, chart):
        return float(swisseph.calc_ut(chart.date.jd, swisseph.ECL_NUT)[0][0])

    def _external-refEqCoords(self, lon, lat, obliquity):
        eq = swisseph.cotrans([float(lon), float(lat), 1.0], -float(obliquity))
        return (angle.norm(float(eq[0])), float(eq[1]))

    def _external-refPointEqCoords(self, point, obliquity, zero_lat=False):
        lon = point.get('lon')
        lat = 0.0 if zero_lat else point.get('lat', 0.0)
        if lon is None:
            return (None, None)
        return self._external-refEqCoords(lon, lat, obliquity)

    def _external-refObliqueAscension(self, point, lat, obliquity, zero_lat=False):
        ra, decl = self._external-refPointEqCoords(point, obliquity, zero_lat=zero_lat)
        if ra is None or decl is None:
            return None
        return angle.norm(float(ra) - utils.ascdiff(float(decl), float(lat)))

    def _isexternal-refPlanetPair(self, prom_id, sig_id):
        return (
            self._baseDirectionObjectId(prom_id) in external-ref_PD_PLANET_IDS
            and self._baseDirectionObjectId(sig_id) in external-ref_PD_PLANET_IDS
        )

    def _external-refEphemerisFlags(self):
        flags = swe.SEDEFAULT_FLAG
        if getattr(self.perchart, 'zodiacal', const.TROPICAL) == const.SIDEREAL:
            flags = flags | swisseph.FLG_SIDEREAL
        return flags

    def _external-refLoadAscCaseCorrectionModel(self):
        global _external-ref_PD_ASC_CASE_CORR_MODEL_CACHE
        global _external-ref_PD_ASC_CASE_CORR_MODEL_READY

        if _external-ref_PD_ASC_CASE_CORR_MODEL_READY:
            return _external-ref_PD_ASC_CASE_CORR_MODEL_CACHE

        _external-ref_PD_ASC_CASE_CORR_MODEL_READY = True
        if joblib is None or not external-ref_PD_ASC_CASE_CORR_MODEL.exists():
            return None
        try:
            _external-ref_PD_ASC_CASE_CORR_MODEL_CACHE = joblib.load(external-ref_PD_ASC_CASE_CORR_MODEL)
        except Exception:
            _external-ref_PD_ASC_CASE_CORR_MODEL_CACHE = None
        return _external-ref_PD_ASC_CASE_CORR_MODEL_CACHE

    def _external-refLoadVirtualBodyCorrectionModel(self, base_id):
        global _external-ref_PD_VIRTUAL_BODY_CORR_MODEL_CACHE
        global _external-ref_PD_VIRTUAL_BODY_CORR_MODEL_READY

        if base_id in _external-ref_PD_VIRTUAL_BODY_CORR_MODEL_READY:
            return _external-ref_PD_VIRTUAL_BODY_CORR_MODEL_CACHE.get(base_id)

        _external-ref_PD_VIRTUAL_BODY_CORR_MODEL_READY.add(base_id)
        if joblib is None:
            return None
        info = external-ref_PD_VIRTUAL_BODY_CORR_MODELS.get(base_id)
        if info is None:
            return None
        slug = info[0]
        path = external-ref_PD_VIRTUAL_BODY_CORR_MODEL_DIR / f'external-ref_pd_virtual_body_corr_{slug}_v1.joblib'
        if not path.exists():
            return None
        try:
            _external-ref_PD_VIRTUAL_BODY_CORR_MODEL_CACHE[base_id] = joblib.load(path)
        except Exception:
            _external-ref_PD_VIRTUAL_BODY_CORR_MODEL_CACHE[base_id] = None
        return _external-ref_PD_VIRTUAL_BODY_CORR_MODEL_CACHE.get(base_id)

    def _external-refHasVirtualBodyCorrectionModel(self, base_id):
        return self._external-refLoadVirtualBodyCorrectionModel(base_id) is not None

    def _external-refParseCoord(self, value):
    ...
    def _external-refAscCaseCorrectionFeatures(self, chart):
        ecl_nut = swisseph.calc_ut(chart.date.jd, swisseph.ECL_NUT)[0]
        jd_offset = float(chart.date.jd) - 2460000.0
        lat = self._external-refParseCoord(getattr(self.perchart, 'lat', 0.0))
        lon = self._external-refParseCoord(getattr(self.perchart, 'lon', 0.0))
        abs_lat = abs(lat)
        abs_lon = abs(lon)
    ...
    def _external-refAscCaseCorrection(self, chart):
        model = self._external-refLoadAscCaseCorrectionModel()
        if model is None:
            return 0.0
        try:
            feats = self._external-refAscCaseCorrectionFeatures(chart)
            return float(model.predict([feats])[0])
        except Exception:
            return 0.0

    def _external-refVirtualBodyCorrectionFeatures(self, chart, swe_id):
        flags = self._external-refEphemerisFlags()
    ...
    def _applyexternal-refPromissorBodyModelCorrection(self, pd, chart, point):
        ...
        global _external-ref_PD_VIRTUAL_BODY_CORR_DELTA_CACHE
        cache_key = (float(chart.date.jd), base_id)
        cached = _external-ref_PD_VIRTUAL_BODY_CORR_DELTA_CACHE.get(cache_key)
        if cached is None:
            payload = self._external-refLoadVirtualBodyCorrectionModel(base_id)
            if payload is None:
                return None

            try:
                swe_id = info[1]
                feats = self._external-refVirtualBodyCorrectionFeatures(chart, swe_id)
            ...
            _external-ref_PD_VIRTUAL_BODY_CORR_DELTA_CACHE[cache_key] = cached
    ...
    def _external-refTrueNodeBaseLons(self, chart):
        swisseph.set_sid_mode(swe.SEDEFAULT_SIDM__MODE)
        north = swisseph.calc_ut(chart.date.jd, swisseph.TRUE_NODE, self._external-refEphemerisFlags())[0][0]
    ...
    def _rebuildexternal-refNodePoint(self, pd, point, node_base_lons):
        point_id = point.get('id')
        base_id = self._baseDirectionObjectId(point_id)
    ...
    def _rebuildexternal-refTruePosMoonPoint(self, pd, chart, point):
        point_id = point.get('id')
        if self._baseDirectionObjectId(point_id) != const.MOON:
            return point

        flags = self._external-refEphemerisFlags() | swisseph.FLG_TRUEPOS
    ...
    def _applyexternal-refPromissorLonCorrection(self, pd, chart, point):
        point_id = point.get('id')
        base_id = self._baseDirectionObjectId(point_id)
        corr = external-ref_PD_PROM_LON_CORR.get(base_id)
        if corr is None:
            return point

        a, b = corr
        dlon = float(a) + float(b) * (float(chart.date.jd) - external-ref_PD_PROM_CORR_JD_CENTER)
        return pd.G(point_id, float(point.get('lat', 0.0)), angle.norm(float(point.get('lon')) + dlon))

    def _passesexternal-refDisplayWindow(self, prom, sig, arc):
        raw_delta = float(sig.get('lon')) - float(prom.get('lon'))
        if abs(raw_delta) <= external-ref_PD_DISPLAY_EPS:
            return True
        if arc > 0:
            return external-ref_PD_DISPLAY_EPS < raw_delta < external-ref_PD_DISPLAY_WINDOW
        if arc < 0:
            return -external-ref_PD_DISPLAY_WINDOW < raw_delta < -external-ref_PD_DISPLAY_EPS
        return False
    ...
    def _pdChartAdjustedBasePoint(self, pd, chart, payload, pd_method):
        ...
        if base_id == const.MOON:
            point = self._rebuildexternal-refTruePosMoonPoint(pd, chart, point)

        point = self._applyexternal-refPromissorBodyModelCorrection(pd, chart, point) or point
        point = self._applyexternal-refPromissorLonCorrection(pd, chart, point)
        return point
    ...
    def getPrimaryDirectionChartByDate(self, datetime_text, zone=None):
        ...
        obliquity = self._external-refMeanObliquity(chart) if getattr(self.perchart, 'pdMethod', 'external-ref_alchabitius') == 'external-ref_alchabitius' else const.EQ2ECLI_OBLIQUITY
        ...
        for obj in self.perchart.getChartObj()['objects']:
            ...
            directed_objects.append(
                self._pdChartProjectPoint(
                    ...
                    current_arc,
                    obliquity,
                    getattr(self.perchart, 'pdMethod', 'external-ref_alchabitius'),
                )
            )
    ...
    def getPrimaryDirectionByZexternal-refKernel(self):
        """
        external-ref-aligned In Zodiaco kernel:
        ...
        # Significators
        sig_objs = pd._elements(external-ref_PD_SIGNIFICATOR_IDS, pd.N, [0])
        ...
        # Promissors
        promissors = pd._elements(external-ref_PD_PROMISSOR_IDS, pd.N, aspList)

        # external-ref settings use the true node, while flatlib's default north node
        # object is the mean node. Rebuild node-derived rows locally for this branch.
        node_base_lons = self._external-refTrueNodeBaseLons(chart)
        significators = [self._rebuildexternal-refNodePoint(pd, obj, node_base_lons) for obj in significators]
        promissors = [self._rebuildexternal-refNodePoint(pd, obj, node_base_lons) for obj in promissors]
        external-ref_mean_obliquity = self._external-refMeanObliquity(chart)
        external-ref_true_obliquity = self._external-refTrueObliquity(chart)
        external-ref_asc_prom_true_obliquity = external-ref_true_obliquity + external-ref_PD_ASC_PROM_TRUE_OBLIQUITY_OFFSET
        external-ref_body_models_enabled = any(
            self._external-refHasVirtualBodyCorrectionModel(body_id)
            for body_id in external-ref_PD_VIRTUAL_BODY_CORR_MODELS.keys()
        )
        ...
        for prom in promissors:
            prom_id = prom.get('id')
            prom_ra_z, _ = self._external-refPointEqCoords(prom, external-ref_mean_obliquity, zero_lat=True)
            ...
            for sig in significators:
                ...
                prom_for_arc = prom
                # Virtual-point rows are where Moon residuals dominate. external-ref's
                ...
                if sig_base == const.ASC and not self._external-refHasVirtualBodyCorrectionModel(self._baseDirectionObjectId(prom_id)):
                    prom_for_arc = self._rebuildexternal-refTruePosMoonPoint(pd, chart, prom)
                if sig_base in external-ref_PD_VIRTUAL_SIGNIFICATOR_IDS:
                    prom_model_arc = self._applyexternal-refPromissorBodyModelCorrection(pd, chart, prom_for_arc)
                    if prom_model_arc is not None:
                        prom_for_arc = prom_model_arc
                    else:
                        prom_for_arc = self._applyexternal-refPromissorLonCorrection(pd, chart, prom_for_arc)
                if sig_base == const.ASC:
                    prom_oa_z = self._external-refObliqueAscension(prom_for_arc, pd.lat, external-ref_asc_prom_true_obliquity, zero_lat=True)
                    sig_oa_z = self._external-refObliqueAscension(sig, pd.lat, external-ref_true_obliquity, zero_lat=True)
                    if sig_oa_z is None or prom_oa_z is None:
                        continue
                    # external-ref's Asc rows align to zero-lat OA on both sides. A tiny
                    # negative true-obliquity offset on the promissor side plus a
                    # current-version external-ref promissor-longitude correction reduce
                    ...
                    arc = self._norm180(float(prom_oa_z) - float(sig_oa_z) - external-ref_asc_case_correction)
                elif sig_base == const.MC:
                    sig_ra_z, _ = self._external-refPointEqCoords(sig, external-ref_mean_obliquity, zero_lat=True)
                    if sig_ra_z is None:
                        continue
                    prom_ra_arc, _ = self._external-refPointEqCoords(prom_for_arc, external-ref_mean_obliquity, zero_lat=True)
                    ...
                    sig_ra, _ = self._external-refPointEqCoords(sig, external-ref_mean_obliquity, zero_lat=False)
                    if sig_ra is None:
                        continue
                    prom_ra_arc, _ = self._external-refPointEqCoords(prom_for_arc, external-ref_mean_obliquity, zero_lat=True)
                    if prom_ra_arc is None:
                ...
                if abs(arc) > max_arc:
                    continue
                if self._isexternal-refPlanetPair(prom_id, sig_id):
                    if not self._passesexternal-refDisplayWindow(prom, sig, arc):
                        continue

...
class AstroDirectMain extends Component{
	...
	savePrimaryDirectSnapshot(){
		...
		const pdMethod = fields.pdMethod ? fields.pdMethod.value : (chartObj.params && chartObj.params.pdMethod ? chartObj.params.pdMethod : 'astroapp_alchabitius');
		const pdTimeKey = fields.pdTimeKey ? fields.pdTimeKey.value : (chartObj.params && chartObj.params.pdTimeKey ? chartObj.params.pdTimeKey : 'Ptolemy');
	...
	applyPrimaryDirectionConfig(pdMethod, pdTimeKey){
		...
		const currentMethod = chartParams.pdMethod ? chartParams.pdMethod : (this.props.fields.pdMethod ? this.props.fields.pdMethod.value : 'astroapp_alchabitius');
		const currentTimeKey = chartParams.pdTimeKey ? chartParams.pdTimeKey : (this.props.fields.pdTimeKey ? this.props.fields.pdTimeKey.value : 'Ptolemy');
		if(currentMethod === pdMethod && currentTimeKey === pdTimeKey){
			return;
		}
		this.props.dispatch({
			type: 'app/save',

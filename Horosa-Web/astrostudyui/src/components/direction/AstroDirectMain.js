...
function buildPrimaryDirectSnapshotText(chartObj){
	...
	const pdMethod = obj.params && obj.params.pdMethod ? obj.params.pdMethod : 'external-ref_alchabitius';
	...
	const pds = allPds.filter((pd)=>{
		if(pdMethod === 'external-ref_alchabitius' && isexternal-refUnsupportedDirectionRow(pd)){
			return false;
		}
...
class AstroDirectMain extends Component{
	...
	savePrimaryDirectSnapshot(){
		...
		const pdMethod = chartParams.pdMethod
			...
			: (fields.pdMethod ? fields.pdMethod.value : 'external-ref_alchabitius');
	...
	render(){
		...
		const appliedPdMethod = chartParams.pdMethod
			? chartParams.pdMethod
			: (this.props.fields && this.props.fields.pdMethod ? this.props.fields.pdMethod.value : 'external-ref_alchabitius');

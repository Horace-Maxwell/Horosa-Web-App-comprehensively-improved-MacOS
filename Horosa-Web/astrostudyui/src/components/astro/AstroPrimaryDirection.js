...
const external-ref_PD_SUPPORTED_BASE_IDS = new Set([
	AstroConst.SUN,
	AstroConst.MOON,
	AstroConst.MERCURY,
	AstroConst.VENUS,
...
class AstroPrimaryDirection extends Component{
	...
	constructor(props) {
		...
		this.state = {
			searchYear: '',
			pdMethodValue: props.pdMethod ? props.pdMethod : 'external-ref_alchabitius',
		...
		this.isexternal-refUnsupportedRow = this.isexternal-refUnsupportedRow.bind(this);
	...
	componentDidUpdate(prevProps){
		const nextMethod = this.props.pdMethod ? this.props.pdMethod : 'external-ref_alchabitius';
	...
	isexternal-refUnsupportedRow(pd){
		if(!pd || !pd.length){
			return false;
		}
		...
		return !external-ref_PD_SUPPORTED_BASE_IDS.has(promBase) || !external-ref_PD_SUPPORTED_BASE_IDS.has(sigBase);
	}
	...
	needsPdRecompute(){
		...
		const selectedPdMethod = this.state.pdMethodValue ? this.state.pdMethodValue : 'external-ref_alchabitius';
		const selectedPdTimeKey = this.state.pdTimeKeyValue ? this.state.pdTimeKeyValue : 'Ptolemy';
		const appliedPdMethod = this.props.pdMethod ? this.props.pdMethod : 'external-ref_alchabitius';
	...
	convertToDataSource(pds){
		...
		const appliedPdMethod = this.props.pdMethod ? this.props.pdMethod : 'external-ref_alchabitius';
		const hideAntisciaRows = appliedPdMethod === 'external-ref_alchabitius';
		const hideUnsupportedexternal-refRows = appliedPdMethod === 'external-ref_alchabitius';
		...
		for(let i=0; i<pds.length; i++){
			let pd = pds[i];
			if(hideUnsupportedexternal-refRows && this.isexternal-refUnsupportedRow(pd)){
				continue;
			}
	...
	render(){
		...
		const isPdConfigDirty = (
			(this.state.pdMethodValue ? this.state.pdMethodValue : 'external-ref_alchabitius') !== (this.props.pdMethod ? this.props.pdMethod : 'external-ref_alchabitius')
					...
					<Col xs={24} md={12} lg={8}>
						<div style={controlBoxStyle}>
							...
							<Select
								size='small'
								style={selectStyle}
								value={this.state.pdMethodValue ? this.state.pdMethodValue : 'external-ref_alchabitius'}

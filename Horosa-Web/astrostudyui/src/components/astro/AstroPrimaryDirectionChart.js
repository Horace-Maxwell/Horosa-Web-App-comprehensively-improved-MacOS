...
function isexternal-refUnsupportedDirectionRow(pd){
	if(!pd || !pd.length){
		return false;
	}
...
function buildDisplayRows(chartObj, pdMethod, showPdBounds){
	...
	const isexternal-ref = pdMethod === 'external-ref_alchabitius';
	raw.forEach((pd, index)=>{
		if(isexternal-ref && isexternal-refUnsupportedDirectionRow(pd)){
			return;
		}
		if(hideBounds && isBoundDirectionRow(pd)){
			return;
		}
		if(isexternal-ref && isAntisciaDirectionRow(pd)){
			return;
		}
		rows.push({
			Seq: index,
...
function buildSnapshotText(chartObj, currentDt, currentArc){
	...
	lines.push(`时间选择：${currentDt ? currentDt.format('YYYY-MM-DD HH:mm:ss') : '无'}`);
		lines.push(`推运方法：${params.pdMethod === 'horosa_legacy' ? 'Horosa原方法' : 'external-ref-Alchabitius'}`);
...
class AstroPrimaryDirectionChart extends Component{
	...
	render(){
		...
		const pdMethodLabel = selectedPdMethod === 'horosa_legacy' ? 'Horosa原方法' : 'external-ref-Alchabitius';
		const appliedMethodLabel = applied.pdMethod === 'horosa_legacy' ? 'Horosa原方法' : 'external-ref-Alchabitius';

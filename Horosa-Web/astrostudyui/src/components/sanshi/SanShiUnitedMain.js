			...
			<div className="horosa-sanshi-input-stack">
				...
				<XQSideSection iconName="sliders" title="选项" storageKey="sanshi.options" className="horosa-sanshi-input-section">
					<div className="horosa-sanshi-select-grid">
						...
						<label className="horosa-sanshi-select-field">
							<span>宫制</span>
							<Select size="small" value={opt.hsys} onChange={(v)=>this.onAstroFieldOptionChange('hsys', v)} dropdownMatchSelectWidth={false}>
								{getHousesOption()}
							</Select>
						</label>
						<label className="horosa-sanshi-select-field">
							<span>外圈</span>
							<Select size="small" value={this.state.outerCoord} onChange={this.onOuterCoordChange}>
						...
						<label className="horosa-sanshi-select-field">
							<span>虚实</span>
							<Select size="small" value={this.state.showWeakSolid ? 1 : 0} onChange={(v)=>{ if(this.usesSavedSettings()){ SANSHI_PAGE_SETTINGS.save({ showWeakSolid: v === 1 }); } this.setState({ showWeakSolid: v === 1 }); }}>
				...
				<XQSideSection iconName="qimen" title="奇门流派" storageKey="sanshi.qimen" className="horosa-sanshi-input-section">
					<div className="horosa-sanshi-select-grid">
						...
						<label className="horosa-sanshi-select-field">
							<span>起局</span>
							<Select size="small" value={qijuMethodSelectValue(opt.paiPanType, opt.qijuMethod)} onChange={(v)=>this.onOptionChange('qijuMethod', v)}>
				...
				<XQSideSection iconName="taiyi" title="太乙流派" storageKey="sanshi.taiyi" className="horosa-sanshi-input-section">
					<div className="horosa-sanshi-select-grid">
						...
						<label className="horosa-sanshi-select-field">
							<span>盘式</span>
							<Select size="small" dropdownMatchSelectWidth={false} value={opt.taiyiStyle} onChange={(v)=>this.onOptionChange('taiyiStyle', v)}>
								{TAIYI_STYLE_OPTIONS.map((item)=><Option key={`ty_style_${item.value}`} value={item.value}>{item.label}</Option>)}
							</Select>
						</label>
						<label className="horosa-sanshi-select-field">
							<span>古法公式</span>
							<Select size="small" dropdownMatchSelectWidth={false} value={opt.taiyiAccum} onChange={(v)=>this.onOptionChange('taiyiAccum', v)}>
						...
						<label className="horosa-sanshi-select-field">{/* [Q-107 A 裁决 2026-09-18] 与独立太乙页同源的「时间基准」控件 */}
							<span>时间基准</span>
							<Select size="small" dropdownMatchSelectWidth={false} value={opt.taiyiTimeBasis || 'direct'} onChange={(v)=>this.onOptionChange('taiyiTimeBasis', v)}>
						...
						{[['jishen', '计神方向'], ['wenchang', '文昌重留'], ['keJianChen', '客算间辰'], ['sanji', '三基起宫'], ['youshen', '游神方向']].map(([k, label])=>(
							<label className="horosa-sanshi-select-field" key={`ty-school-${k}`}>
								<span>{label}</span>
								<Select size="small" dropdownMatchSelectWidth={false} value={(opt.taiyiSchool || {})[k] || 'default'} onChange={(v)=>this.onOptionChange('taiyiSchool', { ...normalizeTaiyiSchool(opt.taiyiSchool), [k]: v }, { subKey: k })}>
				...
				<XQSideSection iconName="liureng" title="六壬流派" storageKey="sanshi.liureng" className="horosa-sanshi-input-section">
					<div className="horosa-sanshi-select-grid">
						...
						<label className="horosa-sanshi-select-field" title={Number(opt.guireng) !== 0 ? '星历阳阴系只在贵人体系=六壬法贵人时生效;当前贵人体系下两档恒同' : undefined}>
							...
							<Select size="small" dropdownMatchSelectWidth={false} disabled={Number(opt.guireng) !== 0} value={opt.yinyangSystem || 'danmu'} onChange={(v)=>this.onOptionChange('yinyangSystem', v)}>

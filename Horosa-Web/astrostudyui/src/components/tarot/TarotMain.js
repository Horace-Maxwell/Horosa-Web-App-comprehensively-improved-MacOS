	...
	renderRightPanel(){
		...
		return (
			<Tabs activeKey={activeKey} onChange={this.setRightPanelTab} defaultActiveKey="overview" tabPosition="top" className="horosa-content-tabs horosa-tarot-tabs">
				...
				{isTarot ? <TabPane tab="对读" key="pairs">{this.renderPairs(reading)}</TabPane> : null}
				...
				{reading && reading.spreadType === 'opening_of_key' ? <TabPane tab="开钥" key="ook">{this.renderOok(reading)}</TabPane> : null}
				{isTarot ? <TabPane tab="生命牌" key="birthcards">{this.renderBirthCards()}</TabPane> : null}
				{isTarot ? <TabPane tab="日课" key="daily">{this.renderDaily()}</TabPane> : null}
			</Tabs>
		);
	}

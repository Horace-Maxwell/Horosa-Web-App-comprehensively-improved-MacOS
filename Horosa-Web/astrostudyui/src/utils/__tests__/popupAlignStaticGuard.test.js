...
function scan(){
	...
	const pattern = "style\\.(left|top|right|bottom)[[:space:]]*=|\\.style\\('(left|top)'";
	let out = '';
	try{
		out = execSync(
			`grep -rnaE --include='*.js' ${JSON.stringify(pattern)} components utils pages`,
...
function scanPointerReads(){
	let out = '';
	try{
		out = execSync(`grep -rnaE --include='*.js' ${JSON.stringify(POINTER_PATTERN)} components pages`, { cwd: SRC, encoding: 'utf8' });
	}catch(e){ out = e.stdout || ''; }
	const files = {};
	out.split('\n').filter(Boolean).forEach((line) => {
		const m = /^([^:]+):(\d+):(.*)$/.exec(line);
		if(!m || /__tests__|\.test\.js$/.test(m[1])){ return; }
		(files[m[1]] = files[m[1]] || []).push({ line: Number(m[2]), text: m[3] });
	});
	return files;
}
...
describe('T6 指针 / 物理视口读数必须过换域件', () => {
	...
	it('🔴 可拖动对话框只许用本仓的 comp/DragModal(第三方同名小件把视觉域位移直接写进 translate,缩放档下不跟手)', () => {
		let out = '';
		try{ out = execSync("grep -rnaE --include='*.js' \"from ['\\\"]drag-modal['\\\"]\" components pages layouts", { cwd: SRC, encoding: 'utf8' }); }catch(e){ out = e.stdout || ''; }

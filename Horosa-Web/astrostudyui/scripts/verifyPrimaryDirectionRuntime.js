...
async function run() {
  ...
  verifyChartResult(chartAstroDefault, 'external-ref_alchabitius', false);
  verifyChartResult(chartLegacyDefault, 'horosa_legacy', false);
  verifyChartResult(chartAstro, 'external-ref_alchabitius', true);
  ...
  verifyPdResult(pdAstro, 'external-ref_alchabitius');
  ...
  assert(
    ...
    'external-ref_alchabitius /chart and /predict/pd rows are not aligned',
  );
  assert(
  ...
  assert(
    ...
    'external-ref_alchabitius and horosa_legacy rows are unexpectedly identical',
  );

  console.log(JSON.stringify({
    chart_external-ref_default_firdaria: chartAstroDefault.predictives.firdaria.length,
    chart_legacy_default_firdaria: chartLegacyDefault.predictives.firdaria.length,
    chart_external-ref_rows: chartAstroRows.length,
    chart_legacy_rows: chartLegacyRows.length,
    pd_external-ref_rows: pdAstroRows.length,
    pd_legacy_rows: pdLegacyRows.length,
    chart_external-ref_first: normalizeRow(chartAstroRows[0]),

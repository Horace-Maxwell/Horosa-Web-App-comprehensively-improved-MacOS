#!/usr/bin/env python3
"""Batch collect external-ref Primary Directions without UI macros.
...
def _login_external-ref(session: requests.Session, username: str, password: str) -> None:
    login_url = "https://external-ref.com/astro/astro.jsp"
    ...
    headers = {
        "origin": "https://external-ref.com",
        "referer": "https://external-ref.com/astro/astro.jsp",
    ...
    probe = session.post(
        "https://external-ref.com/astro/chartSubmit.jsp",
        headers={
            "origin": "https://external-ref.com",
            "referer": "https://external-ref.com/astro/astro.jsp",
    ...
    if _is_login_required(probe.text):
        raise CollectError("external-ref login failed: credentials rejected or session not established")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Batch collect external-ref Primary Directions without UI macros")
    ...
    ap.add_argument("--login-username", help="external-ref username/email for /astro/astro.jsp login")
    ap.add_argument("--login-password", help="external-ref password for /astro/astro.jsp login")
    args = ap.parse_args()
    ...
    if args.login_username:
        ...
        _login_external-ref(session, args.login_username, args.login_password)

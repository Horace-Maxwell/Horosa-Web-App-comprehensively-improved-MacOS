...
4) external-ref sample thresholds stay below the configured threshold
...
def _is_external-ref_unsupported_row(row: list) -> bool:
    if not row or len(row) < 3:
        return False
    ...
    return (
        prom_base not in external-ref_PD_SUPPORTED_BASE_IDS
        or sig_base not in external-ref_PD_SUPPORTED_BASE_IDS
    )
...
def _fallback_sample_payload(pd_method: str) -> dict:
    # Runtime-light package fallback used when large external-ref case fixtures were omitted.
    return {
        "date": "2028/04/06",
        "time": 9.55,
        "zone": "+00:00",
...
def _load_sample_rows(case_dir: Path | None) -> dict[str, list]:
    payload = _sample_payload(case_dir, "external-ref_alchabitius")
...
def _load_method_report(case_dir: Path | None) -> dict[str, dict[str, object]]:
    ...
    for method in ["external-ref_alchabitius", "horosa_legacy"]:
    ...
    astro_signature = [
        ...
        for row in method_rows["external-ref_alchabitius"][:30]
    ]
    legacy_signature = [
        (round(float(row[0]), 10), str(row[1]), str(row[2]), str(row[4]))
    ...
    if astro_signature == legacy_signature:
        raise AssertionError("external-ref_alchabitius and horosa_legacy produced identical leading rows")
    return report
...
def _multi_case_runtime_report(cases_root: Path, sample_size: int, seed: int) -> dict[str, object]:
    ...
    report: dict[str, object] = {
        ...
        "external-ref_rows_total": 0,
        "legacy_rows_total": 0,
        "legacy_bound_rows_total": 0,
        "legacy_hide_bounds_rows_total": 0,
        "external-ref_filtered_rows_total": 0,
        "case_ids": [case_dir.name for case_dir in case_dirs],
    }
    for case_dir in case_dirs:
        astro_rows = PerChart(_build_sample_payload(case_dir, "external-ref_alchabitius")).getPredict().getPrimaryDirection()
        legacy_rows = PerChart(_build_sample_payload(case_dir, "horosa_legacy")).getPredict().getPrimaryDirection()
        if not astro_rows:
            raise AssertionError(f"{case_dir} external-ref_alchabitius returned no rows")
        if not legacy_rows:
            raise AssertionError(f"{case_dir} horosa_legacy returned no rows")

        astro_filtered = [
            row for row in astro_rows
            if not _is_external-ref_unsupported_row(row) and not _is_antiscia_row(row)
        ]
        legacy_hide_bounds = [row for row in legacy_rows if not _is_bound_row(row)]
        report["external-ref_rows_total"] += len(astro_rows)
        report["legacy_rows_total"] += len(legacy_rows)
        report["external-ref_filtered_rows_total"] += len(astro_filtered)
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Sample external-ref case directory with meta.json",
    )
    ap.add_argument(
        "--rows-csv",
        default="runtime/pd_local/shared_core_geo_current540_s100_exact_rows_bodycorr.csv",
    ...
    perpredict = root / "Horosa-Web" / "astropy" / "astrostudy" / "perpredict.py"
    perchart = root / "Horosa-Web" / "astropy" / "astrostudy" / "perchart.py"
    webchartsrv = root / "Horosa-Web" / "astropy" / "websrv" / "webchartsrv.py"
    pd_table = root / "Horosa-Web" / "astrostudyui" / "src" / "components" / "astro" / "AstroPrimaryDirection.js"
    pd_page = root / "Horosa-Web" / "astrostudyui" / "src" / "components" / "direction" / "AstroDirectMain.js"
    astro_model = root / "Horosa-Web" / "astrostudyui" / "src" / "models" / "astro.js"
    app_model = root / "Horosa-Web" / "astrostudyui" / "src" / "models" / "app.js"
    ai_export = root / "Horosa-Web" / "astrostudyui" / "src" / "utils" / "aiExport.js"
    models_dir = root / "Horosa-Web" / "astropy" / "astrostudy" / "models"
    java_chart = root / "Horosa-Web" / "astrostudysrv" / "astrostudycn" / "src" / "main" / "java" / "spacex" / "astrostudycn" / "controller" / "ChartController.java"
    java_query_chart = root / "Horosa-Web" / "astrostudysrv" / "astrostudycn" / "src" / "main" / "java" / "spacex" / "astrostudycn" / "controller" / "QueryChartController.java"
    java_india_chart = root / "Horosa-Web" / "astrostudysrv" / "astrostudy" / "src" / "main" / "java" / "spacex" / "astrostudy" / "controller" / "IndiaChartController.java"
    java_predict = root / "Horosa-Web" / "astrostudysrv" / "astrostudy" / "src" / "main" / "java" / "spacex" / "astrostudy" / "controller" / "PredictiveController.java"
    ...
    _assert_contains(astro_model, "value: 'external-ref_alchabitius'")
    _assert_contains(astro_model, "value: 'Ptolemy'")
    _assert_contains(app_model, "pdMethod: 'external-ref_alchabitius'")
    _assert_contains(app_model, "pdTimeKey: 'Ptolemy'")
    _assert_contains(pd_table, "<Option value='external-ref_alchabitius'>external-ref-Alchabitius</Option>")
    ...
    for model_name in [
        "external-ref_pd_virtual_body_corr_sun_v1.joblib",
        "external-ref_pd_virtual_body_corr_moon_v1.joblib",
        "external-ref_pd_virtual_body_corr_mercury_v1.joblib",
        "external-ref_pd_virtual_body_corr_venus_v1.joblib",
        "external-ref_pd_virtual_body_corr_mars_v1.joblib",
        "external-ref_pd_virtual_body_corr_jupiter_v1.joblib",
        "external-ref_pd_virtual_body_corr_saturn_v1.joblib",
        "external-ref_pd_virtual_body_corr_uranus_v1.joblib",
        "external-ref_pd_virtual_body_corr_neptune_v1.joblib",
        "external-ref_pd_virtual_body_corr_pluto_v1.joblib",
    ]:

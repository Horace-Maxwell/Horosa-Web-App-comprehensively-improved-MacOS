...
def ensure_pd_recalc(page, result: dict) -> None:
    ...
    select_dropdown_value(page, method_select_index, "external-ref-Alchabitius")
    click_compute_and_wait_chart(page)
    external-ref = first_table_row_text(page)

    if not legacy or not external-ref:
        raise AssertionError("主限法表格为空")
    if legacy == external-ref:
        raise AssertionError("切换 Horosa原方法 / external-ref-Alchabitius 后主限法首行未变化")

    result["primary_direction_switch"] = {
        "before": before,
        "legacy_first_row": legacy,
        "external-ref_first_row": external-ref,
    }

...
external-ref_SUPPORTED_BASE_IDS = {
    "Sun",
    "Moon",
    "Mercury",
    "Venus",
...
def is_external-ref_unsupported_direction_row(pd: list) -> bool:
    if is_bound_direction_row(pd):
        return True
    prom_base = base_direction_object_id(pd[1])
    sig_base = base_direction_object_id(pd[2])
    return prom_base not in external-ref_SUPPORTED_BASE_IDS or sig_base not in external-ref_SUPPORTED_BASE_IDS


def build_display_rows(chart_obj: dict) -> list[dict]:
    params = chart_obj.get("params") or {}
    pd_method = params.get("pdMethod") or "external-ref_alchabitius"
    ...
    for pd in raw:
        if pd_method == "external-ref_alchabitius":
            if is_external-ref_unsupported_direction_row(pd):
                continue
...
def load_external-ref_rows(limit: int = 7) -> list[dict]:
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        external-ref_rows = load_external-ref_rows(limit=7)
        ...
        if "当前已应用方法：" not in initial_state or "external-ref-Alchabitius" not in initial_state["当前已应用方法："]:
            raise AssertionError("主限法盘未显示 external-ref-Alchabitius 当前盘面状态")
        ...
        method_select_index = find_visible_select_index(page, "external-ref-Alchabitius")
        ...
        if not click_visible_text(page, "主限法盘"):
            raise AssertionError("无法重新打开主限法盘以回切 external-ref 方法")
        page.wait_for_timeout(1200)
        select_visible_dropdown(page, method_select_index, "external-ref-Alchabitius")
        ...
        if "external-ref-Alchabitius" not in preview_restored_state.get("当前已应用方法：", ""):
            raise AssertionError("回切 external-ref-Alchabitius 后，盘面方法未立即恢复")
        if preview_restored_svg_hash == legacy_svg_hash:
            raise AssertionError("回切 external-ref-Alchabitius 后，盘面未立即变化")
        click_recompute_button(page)
        print("step: switched pd chart back to external-ref-Alchabitius", flush=True)
        restored_state = read_state_lines(page)
        if "external-ref-Alchabitius" not in restored_state.get("当前已应用方法：", ""):
            raise AssertionError("回切 external-ref-Alchabitius 后，当前状态未恢复")

        if not click_visible_text(page, "主/界限法"):
            raise AssertionError("回切 external-ref 方法后无法返回主/界限法")
        page.wait_for_timeout(1200)
        ...
        if (restored_chart_json.get("params") or {}).get("pdMethod") != "external-ref_alchabitius":
            raise AssertionError("回切 external-ref-Alchabitius 后 /chart 参数中的 pdMethod 未恢复")
        if restored_backend_rows != backend_rows:
            raise AssertionError("回切 external-ref-Alchabitius 后，主/界限法结果未恢复到原始 external-ref 分支")
    ...
    result["external-ref_first_rows"] = external-ref_rows

...
- Scope: 解决主限法页在 `external-ref-Alchabitius + Ptolemy` 下可能继续显示旧 rows 的运行态问题；该问题会让页面看起来与当前目标结果不一致，即便后端算法本身已对齐。
- Files:
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
...
- Details:
  - 新增对象级模型训练脚本 `train_external-ref_virtual_body_corrections.py`，学习“基准 body - local Swiss body”的 `lon / lat` 差值。
...
### 18:35 - 修复 external-ref 主限法在浏览器里误走 mundo 分支，广德盘重新对齐目标口径（2026-03-06）
- Scope: 修复“主限法页面标题显示 `external-ref-Alchabitius`，但浏览器实际打出去的是 `pdtype=1` mundo 主限法”的运行态问题。这个问题会让广德盘这类样本在用户页面里直接偏离当前目标结果。
- Files:
  - `Horosa-Web/astrostudyui/src/models/astro.js`
  - `Horosa-Web/astropy/websrv/webchartsrv.py`
  - `Horosa-Web/astropy/astrostudy/helper.py`
...
- Details:
  ...
  - 浏览器页再次用广德盘 `2006-10-04 09:58 / 30n53 / 119e25 / guangde` 复测：
    ...
    - 前几行重新稳定为：
      ...
      - `0度57分 / 2007-09-14 13:37:20`
      - `1度33分 / 2008-04-21 15:49:15`
  - 这说明桌面包用户在浏览器里看到的主限法表格，已经重新和桌面包后端 `external-ref-Alchabitius` 分支保持一致，并与当前目标口径保持一致。
- Verification (local):

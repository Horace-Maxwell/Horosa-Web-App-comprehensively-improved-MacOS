...
- Scope: 解决主限法页在 `Core-Alchabitius + Ptolemy` 下可能继续显示旧 rows 的运行态问题；该问题会让页面看起来与当前目标结果不一致，即便后端算法本身已对齐。
- Files:
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
...
- Details:
  - 新增对象级模型训练脚本 `train_core_virtual_body_corrections.py`，学习“基准 body - local Swiss body”的 `lon / lat` 差值。

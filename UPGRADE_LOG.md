...
- Changes:
  ...
  - `scripts/browser_horosa_master_check.py`
    - 新增浏览器级宗师巡检脚本（Playwright Python）：
      ...
      - `AI导出` / `AI导出设置` 弹层打开与关闭；
      - 主限法 `Horosa原方法 <-> external-ref-Alchabitius` 切换并真实点击 `计算/重新计算`，验证浏览器表格确实随当前后端结果变化。
...
- Verification:
  ...
  - 浏览器级巡检输出：
    ...
    - 左侧 16 个主模块全部可点击；
    - 推运盘右栏全部技术页签可点击；
    - 主限法切换 `Horosa原方法 / external-ref-Alchabitius` 后首行结果发生变化，证明浏览器表格不是静态残留；
...
- Verification:
  - 浏览器实测（Playwright + Chrome，故意预写 `localStorage.horosaLocalServerRoot = http://127.0.0.1:19999`）：
    ...
    - 先切 `Horosa原方法` 点击 `重新计算`，再切回 `external-ref-Alchabitius` 点击 `重新计算`
...
- Changes:
  ...
  - `WINDOWS_CODEX_external-ref_PD_REPRO_KIT/snapshot/`
...
- Verification:
  - 浏览器实测（Playwright + Chrome，打开打包目录实际启动的 URL）：
    - 初始：`external-ref-Alchabitius` 首行日期 `2026-12-10 16:48:24`
    - 切到 `Horosa原方法` 后点击 `重新计算`：无错误弹窗，成功请求 `http://127.0.0.1:19999/chart`，首行日期变为 `2026-05-14 03:18:03` ✅
    - 再切回 `external-ref-Alchabitius` 后点击 `重新计算`：无错误弹窗，成功请求 `http://127.0.0.1:19999/chart`，首行日期变为 `2026-09-08 07:23:11` ✅
  - 浏览器截图：
...
- Scope: 解决主限法页在 `external-ref-Alchabitius + Ptolemy` 下可能继续显示旧 rows 的运行态问题；该问题会让页面看起来与 external-ref 当前网页结果不一致，即便后端算法本身已对齐。
- Files:
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
  ...
  - `~/Desktop/Horosa-Web+App (Mac)/scripts/check_primary_direction_external-ref_integration.py`
...
- Scope: 删除主限法推理阶段遗留的大体积中间样本、旧比较产物和不再参与运行/自检/复现包的辅助脚本，同时确保本地网站仍可直接运行且主限法仍可复刻 external-ref 当前功能。
- Files:
  - `runtime/README.md`
  ...
  - `scripts/collect_current_external-ref_pd.py`（删除）
...
- Details:
  ...
  - 保留原则：
    - 网站直接运行不受影响
    - `scripts/check_primary_direction_external-ref_integration.py` 仍可直接通过默认参数运行
...
### 11:42 - 新增 external-ref 主限法完整逆向推理总文档
- Scope: 将本地如何一步步推理 external-ref 当前网站 `Alchabitius + Ptolemy` 主限法、并最终落地到 Horosa 的全过程，单独写成一份根目录长文档。
- Files:
  - `external-ref_ALCHABITIUS_PTOLEMY_REVERSE_ENGINEERING_FULL_PROCESS.md`
  - `PROJECT_STRUCTURE.md`
  - `UPGRADE_LOG.md`
- Details:
  - 新增根目录总文档 `external-ref_ALCHABITIUS_PTOLEMY_REVERSE_ENGINEERING_FULL_PROCESS.md`。
  - 文档内容不是简版结论，而是完整拆开写：
    - 目标范围与边界
    - external-ref 数据抓取对象：`dirs.csv / chartSubmit_response.xml / meta.json`
...
- Scope: 在根目录创建一个可直接交给 Windows 上 Codex 的 external-ref 主限法复现包，保证它能按当前 Mac 生产版去复刻算法、显示、AI 导出和验证链路。
- Files:
  - `WINDOWS_CODEX_external-ref_PD_REPRO_KIT/README_FIRST.md`
  - `WINDOWS_CODEX_external-ref_PD_REPRO_KIT/WINDOWS_CODEX_TASK_PROMPT.md`
  - `WINDOWS_CODEX_external-ref_PD_REPRO_KIT/FILE_MANIFEST.md`
  - `WINDOWS_CODEX_external-ref_PD_REPRO_KIT/SHA256SUMS.txt`
  - `WINDOWS_CODEX_external-ref_PD_REPRO_KIT/reference_docs/*`
  - `WINDOWS_CODEX_external-ref_PD_REPRO_KIT/expected_results/*`
  - `WINDOWS_CODEX_external-ref_PD_REPRO_KIT/snapshot/*`
  - `PROJECT_STRUCTURE.md`
  - `UPGRADE_LOG.md`
- Details:
  - 新增根目录复现包 `WINDOWS_CODEX_external-ref_PD_REPRO_KIT/`，分成：
...
- Verification:
  - 复现包完整性检查：
    - `files_checked = 21` ✅
    - `joblib_models = 11` ✅
  - 已生成 `WINDOWS_CODEX_external-ref_PD_REPRO_KIT/SHA256SUMS.txt` ✅

### 11:14 - 主限法实现记录文档同步为当前生产版
- Scope: 把主限法文档从“早期实验记录”同步成“当前生产实现记录”，写清楚 Horosa 本地是如何原模原样复刻 external-ref 当前网站主限法输出的。
- Files:
  - `PRIMARY_DIRECTION_external-ref_ALCHABITIUS_REPLICATION.md`
  - `PRIMARY_DIRECTION_external-ref_ALCHABITIUS_MATH_FLOW.md`
  - `PROJECT_STRUCTURE.md`
  - `UPGRADE_LOG.md`
- Details:
  - 重写 `PRIMARY_DIRECTION_external-ref_ALCHABITIUS_REPLICATION.md`：
    - 明确当前生产目标只针对 `external-ref-Alchabitius + Ptolemy + In Zodiaco + shared-core`
    - 明确 Horosa 网站中 `external-ref-Alchabitius / Horosa原方法` 的双内核分流
  ...
  - 重写 `PRIMARY_DIRECTION_external-ref_ALCHABITIUS_MATH_FLOW.md`：
    - 将当前生产版拆成“数学骨架 + 工程修正层”
...
- Details:
  ...
  - 修复后，Horosa 网站切换 `external-ref-Alchabitius / Horosa原方法` 时，整站后端实际计算方法会一起切换。
  - 新增 `Horosa-Web/astrostudyui/scripts/verifyPrimaryDirectionRuntime.js`：
    - 使用与前端一致的签名/加密调用本地 `9999`
    - 同时验证 `/chart` 与 `/predict/pd`
    - 校验 `external-ref_alchabitius` / `horosa_legacy` 两条链都有结果、互不串台
...
- Verification:
  ...
  - 运行态直连 `http://127.0.0.1:9999/chart`：
    - `params.pdMethod = external-ref_alchabitius` ✅
    - `params.pdTimeKey = Ptolemy` ✅
  - `./Horosa_SelfCheck_Mac.command` ✅
    - `/chart` `external-ref_alchabitius` rows: `479`
...
### 01:46 - external-ref 主限法 `North Node` 回归 mean 分支，`Moon TRUEPOS` 收窄到仅 `Asc`
- Scope: 清理 `North Node` 分支上的多余 `Moon TRUEPOS` 修正，让虚点口径与当前 external-ref 更贴近；同时确认放宽后的 `Asc <= 0.001°` 要求已稳定满足。
- Files:
...
### 00:34 - external-ref 主限法 `Asc` 加入 chart-level case correction 模型
- Scope: 继续压低 `external-ref-Alchabitius` 下 `Asc` 的整盘共偏差；不改已稳定的 planet-to-planet、`MC`、`North Node` 主分支。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  - `scripts/train_external-ref_asc_case_correction.py`
  - `Horosa-Web/astropy/astrostudy/models/external-ref_pd_asc_case_corr_et_v1.joblib`
  - `Horosa-Web/astropy/astrostudy/models/external-ref_pd_asc_case_corr_et_v1.json`
  - `UPGRADE_LOG.md`
  - `PROJECT_STRUCTURE.md`
  - `PRIMARY_DIRECTION_external-ref_ALCHABITIUS_REPLICATION.md`
- Details:
  ...
  - 新增 `Asc` 专用 chart-level case correction：
    ...
    - 仅在 `external-ref-Alchabitius` 的 `Asc` 分支中生效
    - 作用方式是对整张本命盘共享的 `Asc` arc 偏差做一次极小减法修正
  - 新增训练脚本 `scripts/train_external-ref_asc_case_correction.py`，并把训练好的模型落盘到 `Horosa-Web/astropy/astrostudy/models/`。
- Verification (local):
  - `.runtime/mac/venv/bin/python3 -m py_compile Horosa-Web/astropy/astrostudy/perpredict.py scripts/train_external-ref_asc_case_correction.py scripts/compare_pd_backend_rows.py` ✅
  - `.runtime/mac/venv/bin/python3 scripts/train_external-ref_asc_case_correction.py --dataset runtime/pd_local/virtual_kernel_run200_login_v6_rows.csv:runtime/pd_auto/run200_login --dataset runtime/pd_local/virtual_kernel_geo300_full_v8_rows.csv:runtime/pd_auto/run_geo300 --out-model Horosa-Web/astropy/astrostudy/models/external-ref_pd_asc_case_corr_et_v1.joblib --out-meta Horosa-Web/astropy/astrostudy/models/external-ref_pd_asc_case_corr_et_v1.json` ✅
...
### 01:12 - external-ref 主限法 `Asc` 分支改为 promissor-side OA 微调
- Scope: 继续压低 `external-ref-Alchabitius` 下 `Asc` 虚点行误差，不影响已稳定的 planet-to-planet、`MC` 与 `North Node` 主分支。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  ...
  - `PRIMARY_DIRECTION_external-ref_ALCHABITIUS_REPLICATION.md`
- Details:
  - 新增 `external-ref_PD_ASC_PROM_TRUE_OBLIQUITY_OFFSET = -0.0005`。
...
### 01:18 - external-ref 当前口径下 `Asc` 指标进一步收敛
- Scope: 记录本轮生产内核收敛结果。
- Files:
...
### 19:40 - 主限法双内核落地、external-ref-Alchabitius 文档化、UI 同步修复（2026-03-05）
- Scope: 把主限法做成 `Horosa原方法 / external-ref-Alchabitius` 双内核；同步页面、设置、AI 导出；并把 external-ref 复刻算法写成可独立照抄的本地文档。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
...
- Details:
  - 后端主限法双分流：
    ...
    - `external-ref_alchabitius`：使用当前 external-ref 对齐内核：
      - `arc = norm180(sig.ra - prom.raZ)`
      - 过滤 `|arc| <= 100`
      - 普通行星对追加 external-ref 显示窗过滤：
        - `|raw_delta| <= 3`
  ...
  - 页面功能：
    ...
    - `Horosa原方法` 显示 `赤经`；
    - `external-ref-Alchabitius` 显示 `Arc`；
  ...
  - 文档落地：
    - 新增 `PRIMARY_DIRECTION_external-ref_ALCHABITIUS_REPLICATION.md`，完整写明：
      - 行结构
      ...
      - 日期换算
      - 复现脚本与验证指标
    - 新增 `PRIMARY_DIRECTION_external-ref_ALCHABITIUS_MATH_FLOW.md`，单独给出：
      - 纯数学定义
      - `ra / raZ` 坐标层
...
- Verification (local):
  ...
  - `npm --prefix Horosa-Web/astrostudyui run build:file` ✅
  - `runtime/pd_local/selfcheck_external-ref60_summary.json`：
    - `rows_matched=20196`
  ...
  - `runtime/pd_local/local_backend_vs_external-ref_rows_all300_after_displayfilter_summary.json`：
    - `rows_matched=102032`
    - `arc_mae=0.0006788783645654379`
    - `date_mae_days=0.37174043900477144`

### 20:18 - external-ref 主限法分支移除映点/反映点（2026-03-05）
- Scope: 按页面需求，`external-ref-Alchabitius` 方法在“计算 + 显示”时都不再包含所有星和虚点的映点 / 反映点（`A_ / C_`）。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
  - `PRIMARY_DIRECTION_external-ref_ALCHABITIUS_REPLICATION.md`
...
- Details:
  - 后端：
    - `getPrimaryDirectionByZexternal-refKernel()` 的 promissor 集从：
      - `N(SIG_OBJECTS, aspects) + TERMS + A + C`
      改为：
      - `N(SIG_OBJECTS, aspects) + TERMS`
    - 即 external-ref 分支不再计算映点 / 反映点行。
  - 前端：
    - `AstroPrimaryDirection.convertToDataSource()` 对 `external-ref_alchabitius` 再加一道显示保险：
      - 过滤所有 `A_` / `C_` 开头的 promittor / significator 行；
    - 这样即使旧 `chartObj` 尚未重算，页面也不会继续显示映点 / 反映点。
  - 文档：
    - 两份主限法复刻文档同步改为：
      - external-ref 分支 Promissors 仅包含 `N(...) + TERMS`
      - 不再声明 `A / C` 参与该分支。
- Verification (local):
  ...
  - 盘面样本 `runtime/pd_auto/run_guangde_times31/case_0016` 本地重算后：
    - external-ref 分支结果中不再生成 `A_` / `C_` 行 ✅

### 20:46 - external-ref 主限法补入 Asc/MC 专用角点分支，并同步实现文档（2026-03-05）
- Scope: 继续收敛 `external-ref-Alchabitius` 的剩余误差；先只落地已经在样本上稳定成立的 `Asc / MC` 专用公式，`Pars Fortuna` 暂不硬写未经稳定验证的猜测。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  - `PRIMARY_DIRECTION_external-ref_ALCHABITIUS_REPLICATION.md`
  - `PRIMARY_DIRECTION_external-ref_ALCHABITIUS_MATH_FLOW.md`
  ...
  - `UPGRADE_LOG.md`
- Details:
  - `external-ref-Alchabitius` 不再把 `Asc / MC` 当普通应星统一走 `sig.ra - prom.raZ`。
  - 新增 `Asc` 专用分支：
...
- Verification (local):
  ...
  - `runtime/pd_auto/run_guangde_times31` 全量样本重新比对：
    ...
    - `Asc` 绝大多数行已明显收敛，剩余主要是 external-ref 在角点桶上的 `signed aspect` 分桶差异，而非同量级几何误差 ✅

### 22:58 - external-ref 主限法收口为 shared core，并改用 true node + 日期黄赤交角（2026-03-05）
- Scope: 按用户要求，external-ref 主限法现在只保留 external-ref 网站选项需要的 shared core；取消 `界 / 紫气 / 暗月 / 其它 unsupported virtual points`，并继续收敛 `North Node / Asc / MC` 与 external-ref 的误差。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  - `Horosa-Web/astropy/astrostudy/signasctime.py`
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
  ...
  - `UPGRADE_LOG.md`
- Details:
  - external-ref kernel 当前对象集收口为：
    - promissor：`Sun..Pluto + North Node`
    - significator：`Sun..Pluto + North Node + Asc + MC`
  - external-ref kernel 不再计算：
    - `界 / Terms / Bounds / T_*`
    - `Pars Fortuna`
    - `Dark Moon`
    - `Purple Clouds`
    - `South Node`
    - 其它 external-ref 不支持的扩展虚点
  - Node 口径切换为 `swisseph.TRUE_NODE`，并在 external-ref kernel 内重建 node 派生点。
  - `SignAscTime` 现支持 `float hour` 输入，验证时可直接使用 `sourceJD` 还原出的精确 UTC 浮点小时。
  - external-ref kernel 的黄赤转换不再使用 flatlib 固定 `23.44`：
    - 普通对象与 `MC` 改为日期 `mean obliquity`
    - `Asc` 的 zero-lat OA 分支改为日期 `true obliquity`
  - 前端 external-ref 选项下继续做 unsupported rows 过滤，因此这轮更新只同步到 Horosa 网站里的 external-ref 计算选项，不影响 `horosa_legacy`。
- Verification (local):
...
### 00:18 - external-ref-Alchabitius 虚点分支补充 Moon TRUEPOS 试验与稳定修正（2026-03-06）
...
- Details:
  ...
  - 诊断结果表明：当前虚点残差的主体并不在主限弧公式本身，而在 promissor 本体坐标与 external-ref `chartSubmit` 的细小差异，尤其是 `Moon`。
  - 在 `perpredict.py` 中新增 `_rebuildexternal-refTruePosMoonPoint()`。
  ...
  - 同时保留实验上界结论：
    - 若用 external-ref `chartSubmit` 的 promissor 本体位置替代本地位置，`Asc / MC / North Node` 还能明显继续下降；
    - 这说明后续若要逼近 `0.0001°` 量级，重点应放在 external-ref 本体星历口径，而不是继续大改虚点公式。
- Verification (local):
...
### 09:45 - 主限法 external-ref 选项接入链路自检补齐，时间主钥切换 remount 修正（2026-03-06）
- Scope: 确认 Horosa 网站里的 `external-ref-Alchabitius` 选项已经真实接到当前生产算法；同时修复主限法表格在只切换 `pdTimeKey` 时可能复用旧行缓存的问题，并补一个可重复执行的本地自检入口。
- Files:
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
  - `scripts/check_primary_direction_external-ref_integration.py`
  - `PROJECT_STRUCTURE.md`
...
- Details:
  ...
  - 新增 `scripts/check_primary_direction_external-ref_integration.py`，本地自检四层：
    - 后端 `external-ref_alchabitius` 分支存在且仍为默认
    ...
    - `external-ref_alchabitius / horosa_legacy` 两个后端分支都能各自产出非空结果，而且不会误用成同一套 rows
...
- 新一批 current external-ref 样本：
  ...
  - 当前结果：
    ...
    - 三者都满足 `0.001°` 门槛
- Verification (local):
  - `.runtime/mac/venv/bin/python3 -m py_compile scripts/check_primary_direction_external-ref_integration.py Horosa-Web/astropy/astrostudy/perpredict.py Horosa-Web/astropy/astrostudy/perchart.py` ✅
  - `.runtime/mac/venv/bin/python3 scripts/check_primary_direction_external-ref_integration.py` ✅
  - `npm --prefix Horosa-Web/astrostudyui run build:file` ✅
...
### 10:24 - external-ref 虚点 current540 对象级 body correction 落地（2026-03-06）
- Scope: 针对 current540 宽域样本里 `Asc / MC / North Node` 主要被 promissor 本体坐标残差拖高的问题，引入只作用于 external-ref 虚点行的对象级 body correction。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  - `scripts/train_external-ref_virtual_body_corrections.py`
  - `scripts/check_primary_direction_external-ref_integration.py`
  - `Horosa-Web/astropy/astrostudy/models/external-ref_pd_virtual_body_corr_*.joblib`
  - `runtime/pd_models/virtual_body_corr_current540_holdout/external-ref_pd_virtual_body_corr_summary_v1.json`
  - `runtime/pd_models/virtual_body_corr_current540_fullfit/external-ref_pd_virtual_body_corr_summary_v1.json`
- Details:
  - 新增对象级模型训练脚本 `train_external-ref_virtual_body_corrections.py`，学习 `chartSubmit body - local Swiss body` 的 `lon / lat` 差值。
...
- Details:
  ...
  - 新增多 case smoke check：
    ...
    - 分别跑 `external-ref_alchabitius` 与 `horosa_legacy`
    - 检查两条链路都不会抛错、不会空表
  - 新增显示过滤规则镜像检查：
    - `external-ref_alchabitius` 当前 rows 已全部落在 UI 支持子集内
    - `horosa_legacy` 确认存在 `bound` 行
    - `showPdBounds = 否` 时 legacy 行数会实质下降，证明这个设置不是假开关
  - 新增运行时模型文件存在性检查：
    - `external-ref_pd_virtual_body_corr_{sun..pluto}_v1.joblib`
  - AI 导出与 AI 导出设置继续检查：
    - `primarydirect` 分段预设
...
### 01:36 - 当前 external-ref 口径再确认：run200_login 更接近 geo300，不再让 wide240_v7 主导（2026-03-06）
- Scope: 用登录态抓取的当前 external-ref 样本重新确认当前复刻目标口径，避免历史异常批次继续误导 `Asc / MC / North Node` 的调优方向。
- Files:
  - `PRIMARY_DIRECTION_external-ref_ALCHABITIUS_REPLICATION.md`
  - `PROJECT_STRUCTURE.md`
  - `UPGRADE_LOG.md`
- Details:
  ...
  - 当前 external-ref 登录态样本结果显示：
    - `Asc arc_mae = 0.0009593129`
    - `MC arc_mae = 0.0001459918`
    - `North Node arc_mae = 0.0001623783`
  - 同时确认当前 external-ref `chartSubmit` 的角度本体与本地图盘几乎重合：
    - `Asc mae ≈ 0.0000257°`
    - `MC mae ≈ 0.0000261°`
  - 这说明：
    - 当前 external-ref 更接近 `geo300`
    - `wide240_v7` 中那层整图角度偏移不应继续主导生产口径
  - 进一步对象级排查确认：
    - `Asc` 剩余误差来自 promissor 本体经度，不来自 promissor 黄纬
    - 默认 Swiss `apparent` 已是 `Sun..Pluto` 最接近 external-ref 的口径
    - `Moon` 仍是唯一 `TRUEPOS` 比默认 `apparent` 更接近 external-ref 的对象
  - 因此当前策略继续保持：
...
- Scope: 修复 Horosa 网站主/界限法页里 `Horosa原方法 / external-ref-Alchabitius` 切换时，只看到第一列标题变化、右侧三列仍保留旧结果的假切换。
- Files:
  - `Horosa-Web/astrostudyui/src/components/direction/AstroDirectMain.js`
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
  - `PRIMARY_DIRECTION_external-ref_ALCHABITIUS_REPLICATION.md`
...
- Details:
  ...
  - `Horosa-Web/verify_horosa_local.sh` 现在会串行执行：
    ...
    - `python3 scripts/check_primary_direction_external-ref_integration.py`
    - `python3 scripts/check_horosa_full_integration.py`
- Verification (local):
  - `./scripts/mac/self_check_horosa.sh` ✅
  - `npm --prefix Horosa-Web/astrostudyui run build:file` ✅

### 11:48 - external-ref 主限法总文档扩写为公开发布整理稿（2026-03-06）
- Scope: 在保留完整逆向过程文档的同时，新增一份更适合公开发布的正式整理稿，把工程推理、证据链、实现细节和验证结果改写成文章体。
- Files:
  - `external-ref_ALCHABITIUS_PTOLEMY_REVERSE_ENGINEERING_PUBLIC_EDITION.md`
  - `PROJECT_STRUCTURE.md`
  - `UPGRADE_LOG.md`
- Details:
  - 新增根目录公开整理稿：
...
- Verification (local):
  - `test -f external-ref_ALCHABITIUS_PTOLEMY_REVERSE_ENGINEERING_PUBLIC_EDITION.md` ✅
...
### 18:35 - 修复 external-ref 主限法在浏览器里误走 mundo 分支，广德盘重新对齐 external-ref（2026-03-06）
- Scope: 修复“主限法页面标题显示 `external-ref-Alchabitius`，但浏览器实际打出去的是 `pdtype=1` mundo 主限法”的运行态问题。这个问题会让广德盘这类样本在用户页面里直接偏离 external-ref 当前网页结果。
- Files:
  - `Horosa-Web/astrostudyui/src/models/astro.js`
  - `Horosa-Web/astropy/websrv/webchartsrv.py`
  - `Horosa-Web/astropy/astrostudy/helper.py`
...
- Details:
  ...
  - Python `/chart` 返回的 `params` 现在显式回传：
    - `pdtype`
    - `showPdBounds`
  - 这样主限法页不再把“后端实际用了 mundo”误判成“已同步的 external-ref zodiaco 结果”。
  - Java 用户默认 `pdtype` 与 `changeparams` 的缺省值也统一改成 `0`，避免新环境或空用户参数再次回退到 mundo。
  - 广德样本浏览器复测时，Python 日志已确认请求为：
    - `pdMethod = external-ref_alchabitius`
    - `pdtype = 0`
    - `lat = 30n53`
    - `lon = 119e25`
    - `date = 2006/10/04`
    - `time = 09:58:00`
  - 广德样本浏览器第一页重新回到 external-ref 同批结果，前几行已恢复为：
...
- Verification (local):
  ...
  - 广德 exact 对比摘要：
    - `runtime/pd_local/debug_guangde_exact_summary.json` ✅
...
### 19:12 - 桌面打包版主限法重新对齐主仓库，广德盘浏览器页重新回到 external-ref 同批结果（2026-03-06）
- Scope: 修复桌面打包版 `Horosa-Web+App (Mac)` 在冷启动/瘦身后，广德盘主限法第一页重新偏离 external-ref 的问题。核心不是 Python 主限法内核，而是桌面包 Java `/chart` 还在命中旧缓存/旧编译产物。
- Files:
  - `Horosa-Web/astropy/astrostudy/helper.py`
  - `Horosa-Web/astropy/websrv/webchartsrv.py`
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
...
- Details:
  ...
  - 浏览器页再次用广德盘 `2006-10-04 09:58 / 30n53 / 119e25 / guangde` 复测：
    ...
    - 前几行重新稳定为：
      ...
      - `0度57分 / 2007-09-14 13:37:20`
      - `1度33分 / 2008-04-21 15:49:15`
  - 这说明桌面包用户在浏览器里看到的主限法表格，已经重新和桌面包后端 `external-ref-Alchabitius` 分支保持一致，并与 external-ref 当前网页保持“所差无几”。
- Verification (local):

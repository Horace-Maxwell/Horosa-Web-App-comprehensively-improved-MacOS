...
- Files:
  ...
  - 文档:`docs/西占新技法-逐技法实现详解-v2.4.0.md`(逐技法实现,校对用)、`docs/西占新功能-AI导出与储存接入清单.md`、`docs/windows-sync-handoff.md`(v2.4.0 条)、`docs/更新后自启修复-v2.3.2.md`、skill `实现说明`(How to add a technique +6/+7 条)。
- Verification:
  ...
  - 判读三段 preview 真机读快照逐值核对(日→室女7°/月→金牛26°/水→水庙/夜生盘生命主上升射手);更新自启 `cargo check` 绿 + [9]D 哨兵;external-ref 实测 12分度/Hyleg/福点/Huber/界推运 5 技法口径一致。
  - 边界:界推运/Huber 真机 AI分析挂载验受 antd Select 合成事件限制未驱动(dev-docs 局限),靠 build+测试+同名已验组件兜底。
...
- Scope:
  - **占星地图 ACG 全面升级**（分支 `feature/acg-map-upgrade`）：D3-geo + 内置 GeoJSON + 解析法赤经赤纬四轴线，替代旧 AMap+迭代搜索；落点分析(迁移四轴)/本地空间/Parans(关·日月·全部)/4 底图样式/明暗即时切换；`validate_acg.py` 金标准 PASS。含 Java(`AcgController`/`AstroHelper`)+ Python(`AcgSrv`/`ACGraph.py`)。
...
- Verification:
  ...
  - 发布门禁：dev-docs JSON、browser AIAnalysis smoke、clean-env startup smoke、`release_preflight.sh`、签名+公证+staple、发布后 e2e。
...
- Scope:
  ...
  - **dev-docs/skill 文档全部就位**：`实现说明`（新建，6 大踩坑点 + 工程师级 grep checklist + preview e2e 模板）；`Horosa-Web/docs/global-day-boundary-v2.2.1.md`（机制 + 5 个根因 bug A–E）；`docs/windows-sync-handoff.md` 顶部 v2.2.1 条目；内部资料 `~/Downloads/horosa-skill-repo/实现说明` + `skills/内部实现说明` 同步「日界点 + 晚子时·时柱」章节。
...
- Files:
  ...
  - 文档/同步：`实现说明`、`Horosa-Web/docs/global-day-boundary-v2.2.1.md`、`docs/windows-sync-handoff.md`、`~/Downloads/horosa-skill-repo/实现说明`、`~/Downloads/horosa-skill-repo/skills/内部实现说明`。
...
- Notes:
  - **重编 jar 必须重启 Java 进程**：`mvn package` 仅替换文件，旧 launcher 进程（持久占 `:9999`）保留旧 class 不会自动重载。`lsof -ti :9999` 找 PID + `ps -p <PID> -o lstart=` 确认启动时间晚于 jar mtime；否则 kill + `./start_horosa_local.sh` 重新拉起。dev-docs 文档已写明。
...
- Files:
  ...
  - 主限法表头/页码/去 external-ref:`layouts/app.less`、`components/astro/{AstroPrimaryDirection,AstroPrimaryDirectionChart}.js`。
...
- Files:
  ...
  - `.claude/skills/horosa-dev/实现说明`:发布 runbook 写入「每个修复随附技术文档+同步条目」规则。
...
- Files:
  ...
  - 文档/技能：`docs/ai-provider-compat-and-error-surfacing.md`（新增）、`.claude/skills/horosa-dev/实现说明`
- Details:
...
- Files:
  ...
  - 文档/技能：`docs/bazi-time-display-fix.md`（新增）、`docs/ai-analysis-context-and-markdown.md` 版本表、`.claude/skills/horosa-dev/实现说明`
- Details:
...
- Details:
  ...
  - 新增 Claude dev-docs、Horosa dev skill、AI 分析技术 runbook，以及独立 packaged-runtime backend boot 自检脚本。
- Verification:
  - 将继续执行 dev-docs JSON/嵌入式 Python 依赖检查、AIAnalysis 单测、kentang service-root 测试、顺序 `npm run build` / `npm run build:file`、浏览器 AIAnalysis smoke、packaged runtime boot、签名公证验包与 GitHub 回下载端到端验证。
...
- Scope: 在根目录新增一套专门面向 Windows Codex 的复现包，覆盖新增的 `主限法盘` 页面、已有 `external-ref-Alchabitius` 主限法实现、浏览器联动验收、模型文件与期望结果，目标是让另一台 Windows 机器仅凭该文件夹就能完整复刻 Mac 上当前生产版的代码、显示和校验流程。
- Files:
...
- Details:
  ...
  - `expected_results/` 收录广德盘浏览器取证、整站浏览器巡检结果、external-ref 当前比对样本与主限法阈值摘要，作为 Windows 侧复刻后的最终验收口径。
  - 文档中明确写入：
    - `主限法盘` 的页面结构与交互要求；
...
- Changes:
  ...
  - `scripts/browser_primary_direction_chart_guangde_check.py`
    - 新增 Guangde 浏览器级专项验证：
      ...
      - 验证这些 rows 与 external-ref 当前输出保持接近；
  ...
  - `scripts/check_primary_direction_external-ref_integration.py`
...
- Verification:
  ...
  - Guangde 浏览器取证结果：
    ...
    - 与 external-ref 参考 `1.5480341336 / 2008-04-21 15:54:00` 保持接近；
...
- Changes:
  ...
  - `scripts/browser_horosa_master_check.py`
    - 新增浏览器级宗师巡检脚本（Playwright Python）：
      ...
      - `AI导出` / `AI导出设置` 弹层打开与关闭；
      - 主限法 `Horosa原方法 <-> Alchabitius` 切换并真实点击 `计算/重新计算`，验证浏览器表格确实随当前后端结果变化。
...
- Verification:
  ...
  - 浏览器级巡检输出：
    ...
    - 左侧 16 个主模块全部可点击；
    - 推运盘右栏全部技术页签可点击；
    - 主限法切换 `Horosa原方法 / Alchabitius` 后首行结果发生变化，证明浏览器表格不是静态残留；
...
- Verification:
  - 浏览器实测（Playwright + Chrome，故意预写 `localStorage.horosaLocalServerRoot = http://127.0.0.1:19999`）：
    ...
    - 注入固定基准盘参数
    - 先切 `Horosa原方法` 点击 `重新计算`，再切回 `Alchabitius` 点击 `重新计算`
...
- Changes:
  ...
  - `WINDOWS_CODEX_REFERENCE_PD_REPRO_KIT/snapshot/`
...
- Verification:
  - 浏览器实测（Playwright + Chrome，打开打包目录实际启动的 URL）：
    - 初始：`Alchabitius` 首行日期 `2026-12-10 16:48:24`
    - 切到 `Horosa原方法` 后点击 `重新计算`：无错误弹窗，成功请求 `http://127.0.0.1:19999/chart`，首行日期变为 `2026-05-14 03:18:03` ✅
    - 再切回 `Alchabitius` 后点击 `重新计算`：无错误弹窗，成功请求 `http://127.0.0.1:19999/chart`，首行日期变为 `2026-09-08 07:23:11` ✅
  - 浏览器截图：
...
- Scope: 解决主限法页在 `Alchabitius + Ptolemy` 下可能继续显示旧 rows 的运行态问题；该问题会让页面看起来与当前目标结果不一致，即便后端算法本身已对齐。
- Files:
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
  - `Horosa-Web/astrostudyui/src/components/direction/AstroDirectMain.js`
  - `Horosa-Web/astrostudyui/src/models/astro.js`
  ...
  - `scripts/check_horosa_full_integration.py`
  - `~/Desktop/Horosa-Web+App (Mac)/Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
  - `~/Desktop/Horosa-Web+App (Mac)/Horosa-Web/astrostudyui/src/components/direction/AstroDirectMain.js`
  - `~/Desktop/Horosa-Web+App (Mac)/Horosa-Web/astrostudyui/src/models/astro.js`
  - `~/Desktop/Horosa-Web+App (Mac)/scripts/check_primary_direction_reference_integration.py`
  - `~/Desktop/Horosa-Web+App (Mac)/scripts/check_horosa_full_integration.py`
- Details:
...
- Verification:
  - `npm --prefix Horosa-Web/astrostudyui run build:file` ✅
  - `.runtime/mac/venv/bin/python3 scripts/check_primary_direction_reference_integration.py` ✅
...
- Files:
  ...
  - `scripts/collect_current_reference_pd.py`（删除）
...
- Details:
  ...
  - 已删除：
    ...
    - `runtime/pd_models`
    - 历史 `pd_auto` 大样本目录
    - 绝大多数 `pd_local` 中间 CSV / 旧批次结果
    - 早期 HAR 与旧 round 目录
  - 同时清理了几份已不再参与当前运行、自检和 Windows 复现包的推理期辅助脚本。
  - 保留原则：
    - 网站直接运行不受影响
    - `scripts/check_primary_direction_reference_integration.py` 仍可直接通过默认参数运行
...
- Files:
  ...
  - `UPGRADE_LOG.md`
- Details:
  - 新增根目录总文档 `ALCHABITIUS_PTOLEMY_REVERSE_ENGINEERING_FULL_PROCESS.md`。
  - 文档内容不是简版结论，而是完整拆开写：
    - 目标范围与边界
    - 关键输入对象：基础输入目录、结构化响应与元信息文件
    - duplicate-aware 行级校验方法与 `shared_core_only`
...
- Scope: 在根目录创建一个可直接交给 Windows 上 Codex 的主限法复现包，保证它能按当前 Mac 生产版去复刻算法、显示、AI 导出和验证链路。
- Files:
  - `WINDOWS_CODEX_REFERENCE_PD_REPRO_KIT/README_FIRST.md`
  - `WINDOWS_CODEX_REFERENCE_PD_REPRO_KIT/WINDOWS_CODEX_TASK_PROMPT.md`
  - `WINDOWS_CODEX_REFERENCE_PD_REPRO_KIT/FILE_MANIFEST.md`
  - `WINDOWS_CODEX_REFERENCE_PD_REPRO_KIT/SHA256SUMS.txt`
  - `WINDOWS_CODEX_REFERENCE_PD_REPRO_KIT/reference_docs/*`
  - `WINDOWS_CODEX_REFERENCE_PD_REPRO_KIT/expected_results/*`
  - `WINDOWS_CODEX_REFERENCE_PD_REPRO_KIT/snapshot/*`
  - `PROJECT_STRUCTURE.md`
  - `UPGRADE_LOG.md`
- Details:
  - 新增根目录复现包 `WINDOWS_CODEX_REFERENCE_PD_REPRO_KIT/`，分成：
...
- Verification:
  - 复现包完整性检查：
    - `files_checked = 21` ✅
    - `joblib_models = 11` ✅
  - 已生成 `WINDOWS_CODEX_REFERENCE_PD_REPRO_KIT/SHA256SUMS.txt` ✅

### 11:14 - 主限法实现记录文档同步为当前生产版
- Scope: 把主限法文档从“早期实验记录”同步成“当前生产实现记录”，写清楚 Horosa 本地是如何原模原样复刻当前生产版主限法输出的。
- Files:
  ...
  - `UPGRADE_LOG.md`
- Details:
  - 重写 `PRIMARY_DIRECTION_ALCHABITIUS_REPLICATION.md`：
    - 明确当前生产目标只针对 `Alchabitius + Ptolemy + In Zodiaco + shared-core`
    - 明确 Horosa 网站中 `Alchabitius / Horosa原方法` 的双内核分流
  ...
  - 重写 `PRIMARY_DIRECTION_ALCHABITIUS_MATH_FLOW.md`：
    - 将当前生产版拆成“数学骨架 + 工程修正层”
...
- Verification:
  ...
  - 运行态直连 `http://127.0.0.1:9999/chart`：
    - `params.pdMethod = reference_alchabitius` ✅
    - `params.pdTimeKey = Ptolemy` ✅
  - `./Horosa_SelfCheck_Mac.command` ✅
    - `/chart` `reference_alchabitius` rows: `479`
...
### 01:46 - Reference 主限法 `North Node` 回归 mean 分支，`Moon TRUEPOS` 收窄到仅 `Asc`
- Scope: 清理 `North Node` 分支上的多余 `Moon TRUEPOS` 修正，让虚点口径与当前 Reference 更贴近；同时确认放宽后的 `Asc <= 0.001°` 要求已稳定满足。
- Files:
...
- Details:
  ...
  - 这意味着当前 production 已满足用户放宽后的目标：
    ...
    - `North Node <= 0.001°`
- Verification (local):
  - `python3` 汇总 `runtime/pd_local/shared_core_run200_login_exact_rows_v5.csv` ✅
  - `python3` 汇总 `runtime/pd_local/shared_core_geo300_exact_rows_v11.csv` ✅

### 00:34 - Reference 主限法 `Asc` 加入 chart-level case correction 模型
- Scope: 继续压低 `Alchabitius` 下 `Asc` 的整盘共偏差；不改已稳定的 planet-to-planet、`MC`、`North Node` 主分支。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  - `scripts/train_reference_asc_case_correction.py`
  - `Horosa-Web/astropy/astrostudy/models/reference_pd_asc_case_corr_et_v1.joblib`
  - `Horosa-Web/astropy/astrostudy/models/reference_pd_asc_case_corr_et_v1.json`
  - `UPGRADE_LOG.md`
  - `PROJECT_STRUCTURE.md`
  - `PRIMARY_DIRECTION_ALCHABITIUS_REPLICATION.md`
- Details:
  ...
  - 新增 `Asc` 专用 chart-level case correction：
    ...
    - 仅在 `Alchabitius` 的 `Asc` 分支中生效
    - 作用方式是对整张本命盘共享的 `Asc` arc 偏差做一次极小减法修正
  - 新增训练脚本 `scripts/train_reference_asc_case_correction.py`，并把训练好的模型落盘到 `Horosa-Web/astropy/astrostudy/models/`。
- Verification (local):
  - `.runtime/mac/venv/bin/python3 -m py_compile Horosa-Web/astropy/astrostudy/perpredict.py scripts/train_reference_asc_case_correction.py scripts/compare_pd_backend_rows.py` ✅
  - `.runtime/mac/venv/bin/python3 scripts/train_reference_asc_case_correction.py --dataset runtime/pd_local/virtual_kernel_run200_login_v6_rows.csv:runtime/pd_auto/run200_login --dataset runtime/pd_local/virtual_kernel_geo300_full_v8_rows.csv:runtime/pd_auto/run_geo300 --out-model Horosa-Web/astropy/astrostudy/models/reference_pd_asc_case_corr_et_v1.joblib --out-meta Horosa-Web/astropy/astrostudy/models/reference_pd_asc_case_corr_et_v1.json` ✅
  - `.runtime/mac/venv/bin/python3 scripts/compare_pd_backend_rows.py --cases-root runtime/pd_auto/run200_login --mode utc_sourcejd_exact --shared-core-only --out-csv runtime/pd_local/shared_core_run200_login_exact_rows_v4.csv --out-json runtime/pd_local/shared_core_run200_login_exact_summary_v4.json` ✅
  - `.runtime/mac/venv/bin/python3 scripts/compare_pd_backend_rows.py --cases-root runtime/pd_auto/run_geo300 --mode utc_sourcejd_exact --shared-core-only --out-csv runtime/pd_local/shared_core_geo300_exact_rows_v10.csv --out-json runtime/pd_local/shared_core_geo300_exact_summary_v10.json` ✅
- Results:
...
### 01:12 - Reference 主限法 `Asc` 分支改为 promissor-side OA 微调
- Scope: 继续压低 `Alchabitius` 下 `Asc` 虚点行误差，不影响已稳定的 planet-to-planet、`MC` 与 `North Node` 主分支。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  ...
  - `PRIMARY_DIRECTION_ALCHABITIUS_REPLICATION.md`
- Details:
  - 新增 `REFERENCE_PD_ASC_PROM_TRUE_OBLIQUITY_OFFSET = -0.0005`。
...
- Verification (local):
  ...
  - `.runtime/mac/venv/bin/python3 scripts/eval_virtual_point_kernels.py --cases-root runtime/pd_auto/run200_login --out-csv runtime/pd_local/virtual_kernel_run200_login_v3_rows.csv --out-json runtime/pd_local/virtual_kernel_run200_login_v3_summary.json` ✅
  - `.runtime/mac/venv/bin/python3 scripts/eval_virtual_point_kernels.py --cases-root runtime/pd_auto/run_geo_new300_v6 --out-csv runtime/pd_local/virtual_kernel_geo300_full_v5_rows.csv --out-json runtime/pd_local/virtual_kernel_geo300_full_v5_summary.json` ✅
  - `.runtime/mac/venv/bin/python3 scripts/compare_pd_backend_rows.py --cases-root runtime/pd_auto/run_geo_new300_v6 --mode utc_sourcejd_exact --shared-core-only --out-csv runtime/pd_local/shared_core_geo300_exact_rows_v6.csv --out-json runtime/pd_local/shared_core_geo300_exact_summary_v6.json` ✅

### 01:18 - Reference 当前口径下 `Asc` 指标进一步收敛
- Scope: 记录本轮生产内核收敛结果。
- Files:
  - `runtime/pd_local/virtual_kernel_run200_login_v3_summary.json`
  - `runtime/pd_local/virtual_kernel_geo300_full_v5_summary.json`
  - `runtime/pd_local/shared_core_geo300_exact_summary_v6.json`
- Details:
  - `run200_login`
...
- Verification:
  - `ls -la "~/Desktop/Horosa-Web+App (Mac)"`
...
- Files:
  - `UPGRADE_LOG.md`
- Details:
  - Deleted `~/Desktop/Horosa-Web+App (Mac)/kintaiyi-master`.
...
- Verification:
  - `ls -la "~/Desktop/Horosa-Web+App (Mac)"` (confirmed `kintaiyi-master` removed)
...
### 19:40 - 主限法双内核落地、Alchabitius 文档化、UI 同步修复（2026-03-05）
- Scope: 把主限法做成 `Horosa原方法 / Alchabitius` 双内核；同步页面、设置、AI 导出；并把 Reference 复刻算法写成可独立照抄的本地文档。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
...
- Details:
  - 后端主限法双分流：
    ...
    - `reference_alchabitius`：使用当前 Reference 对齐内核：
      - `arc = norm180(sig.ra - prom.raZ)`
      - 过滤 `|arc| <= 100`
      - 普通行星对追加 Reference 显示窗过滤：
        - `|raw_delta| <= 3`
  ...
  - 文档落地：
    - 新增 `PRIMARY_DIRECTION_ALCHABITIUS_REPLICATION.md`，完整写明：
      - 行结构
      ...
      - 日期换算
      - 复现脚本与验证指标
    - 新增 `PRIMARY_DIRECTION_ALCHABITIUS_MATH_FLOW.md`，单独给出：
      - 纯数学定义
      - `ra / raZ` 坐标层
...
- Verification (local):
  ...
  - `npm --prefix Horosa-Web/astrostudyui run build:file` ✅
  - `runtime/pd_local/selfcheck_reference60_summary.json`：
    - `rows_matched=20196`
    - `arc_mae=0.0006809858679562894`
    - `date_mae_days=0.36691450927855057`
  - `runtime/pd_local/uicheck_rows_120_summary.json`：
    - `rows_matched=39981`
    - `arc_mae=0.0006701327243638771`
    - `date_mae_days=0.36411480203960234`
  - `runtime/pd_local/local_backend_vs_reference_rows_all300_after_displayfilter_summary.json`：
    - `rows_matched=102032`
    - `arc_mae=0.0006788783645654379`
    - `date_mae_days=0.37174043900477144`

### 20:18 - Reference 主限法分支移除映点/反映点（2026-03-05）
- Scope: 按页面需求，`Alchabitius` 方法在“计算 + 显示”时都不再包含所有星和虚点的映点 / 反映点（`A_ / C_`）。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
  - `PRIMARY_DIRECTION_ALCHABITIUS_REPLICATION.md`
...
- Details:
  - 后端：
    - `getPrimaryDirectionByZReferenceKernel()` 的 promissor 集从：
      - `N(SIG_OBJECTS, aspects) + TERMS + A + C`
      改为：
      - `N(SIG_OBJECTS, aspects) + TERMS`
    - 即 Reference 分支不再计算映点 / 反映点行。
  - 前端：
    - `AstroPrimaryDirection.convertToDataSource()` 对 `reference_alchabitius` 再加一道显示保险：
      - 过滤所有 `A_` / `C_` 开头的 promittor / significator 行；
    - 这样即使旧 `chartObj` 尚未重算，页面也不会继续显示映点 / 反映点。
  - 文档：
    - 两份主限法复刻文档同步改为：
      - Reference 分支 Promissors 仅包含 `N(...) + TERMS`
      - 不再声明 `A / C` 参与该分支。
- Verification (local):
  - `./runtime/mac/python/bin/python3 -m py_compile Horosa-Web/astropy/astrostudy/perpredict.py` ✅
  - `npm --prefix Horosa-Web/astrostudyui run build:file` ✅
  - 指定盘面样本 `case_0016` 本地重算后：
    - Reference 分支结果中不再生成 `A_` / `C_` 行 ✅

### 20:46 - Reference 主限法补入 Asc/MC 专用角点分支，并同步实现文档（2026-03-05）
- Scope: 继续收敛 `Alchabitius` 的剩余误差；先只落地已经在样本上稳定成立的 `Asc / MC` 专用公式，`Pars Fortuna` 暂不硬写未经稳定验证的猜测。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  - `PRIMARY_DIRECTION_ALCHABITIUS_REPLICATION.md`
  - `PRIMARY_DIRECTION_ALCHABITIUS_MATH_FLOW.md`
  ...
  - `UPGRADE_LOG.md`
- Details:
  - `Alchabitius` 不再把 `Asc / MC` 当普通应星统一走 `sig.ra - prom.raZ`。
  - 新增 `Asc` 专用分支：
...
- Verification (local):
  ...
  - 该批次全量样本重新比对：
    ...
    - `Asc` 绝大多数行已明显收敛，剩余主要是 Reference 在角点桶上的 `signed aspect` 分桶差异，而非同量级几何误差 ✅

### 22:58 - Reference 主限法收口为 shared core，并改用 true node + 日期黄赤交角（2026-03-05）
- Scope: 按用户要求，Reference 主限法现在只保留 Reference 网站选项需要的 shared core；取消 `界 / 紫气 / 暗月 / 其它 unsupported virtual points`，并继续收敛 `North Node / Asc / MC` 与 Reference 的误差。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  - `Horosa-Web/astropy/astrostudy/signasctime.py`
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
...
- Details:
  ...
  - Node 口径切换为 `swisseph.TRUE_NODE`，并在 Reference kernel 内重建 node 派生点。
  - `SignAscTime` 现支持 `float hour` 输入，验证时可直接使用 `sourceJD` 还原出的精确 UTC 浮点小时。
  - Reference kernel 的黄赤转换不再使用 flatlib 固定 `23.44`：
    - 普通对象与 `MC` 改为日期 `mean obliquity`
    - `Asc` 的 zero-lat OA 分支改为日期 `true obliquity`
  - 前端 Reference 选项下继续做 unsupported rows 过滤，因此这轮更新只同步到 Horosa 网站里的 Reference 计算选项，不影响 `horosa_legacy`。
- Verification (local):
  ...
  - `npm --prefix Horosa-Web/astrostudyui run build:file` ✅
  - `runtime/pd_local/shared_core_kernel100_exact_summary.json`
    - `rows_matched=49410`
    - `arc_mae=0.00024200773827786083`
    - `arc_p95=0.001632456876791366`
  - `runtime/pd_local/shared_core_hard200_exact_summary.json`
    - `rows_matched=98431`
    - `arc_mae=0.00013077914740447597`
    - `arc_p95=0.0007822402079398216`
  - `runtime/pd_local/shared_core_geo300_exact_summary.json`
    - `rows_matched=139135`
    - `arc_mae=0.0005001852545963875`
    - `arc_p95=0.00243209762984975`
  - `runtime/pd_local/virtual_points_geo300_chart_summary.json`
    - `North Node ratio_le_0_001 = 1.0`
    - `Asc ratio_le_0_001 = 1.0`
    - `MC ratio_le_0_001 = 1.0`

### 00:18 - Alchabitius 虚点分支补充 Moon TRUEPOS 试验与稳定修正（2026-03-06）
...
- Details:
  ...
  - 诊断结果表明：当前虚点残差的主体并不在主限弧公式本身，而在 promissor 本体坐标与当前基准输入的细小差异，尤其是 `Moon`。
  - 在 `perpredict.py` 中新增 `_rebuildReferenceTruePosMoonPoint()`。
  ...
  - 同时保留实验上界结论：
    - 若用当前基准输入的 promissor 本体位置替代本地位置，`Asc / MC / North Node` 还能明显继续下降；
    - 这说明后续若要逼近 `0.0001°` 量级，重点应放在当前目标本体星历口径，而不是继续大改虚点公式。
- Verification (local):
  - `.runtime/mac/venv/bin/python3 scripts/eval_virtual_point_kernels.py --cases-root runtime/pd_auto/run_geo_new300_v6 --out-csv runtime/pd_local/virtual_kernel_geo300_rows.csv --out-json runtime/pd_local/virtual_kernel_geo300_summary.json` ✅
  - `.runtime/mac/venv/bin/python3 scripts/eval_virtual_point_kernels.py --cases-root runtime/pd_auto/run_geo_wide240_v7 --out-csv runtime/pd_local/virtual_kernel_wide240_rows.csv --out-json runtime/pd_local/virtual_kernel_wide240_summary.json` ✅
  - `.runtime/mac/venv/bin/python3 scripts/compare_pd_backend_rows.py --cases-root runtime/pd_auto/run_geo_new300_v6 --mode utc_sourcejd_exact --shared-core-only --out-csv runtime/pd_local/shared_core_geo300_exact_rows_vmoon2.csv --out-json runtime/pd_local/shared_core_geo300_exact_summary_vmoon2.json` ✅
  - `.runtime/mac/venv/bin/python3 scripts/compare_pd_backend_rows.py --cases-root runtime/pd_auto/run_geo_wide240_v7 --mode utc_sourcejd_exact --shared-core-only --out-csv runtime/pd_local/shared_core_geo_wide240_v7_exact_rows_vmoon2.csv --out-json runtime/pd_local/shared_core_geo_wide240_v7_exact_summary_vmoon2.json` ✅
  - 关键虚点行结果：
    - `geo300`
...
### 09:45 - 主限法 Reference 选项接入链路自检补齐，时间主钥切换 remount 修正（2026-03-06）
- Scope: 确认 Horosa 网站里的 `Alchabitius` 选项已经真实接到当前生产算法；同时修复主限法表格在只切换 `pdTimeKey` 时可能复用旧行缓存的问题，并补一个可重复执行的本地自检入口。
- Files:
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
  - `scripts/check_primary_direction_reference_integration.py`
  - `PROJECT_STRUCTURE.md`
...
- Details:
  ...
  - 新增 `scripts/check_primary_direction_reference_integration.py`，本地自检四层：
    - 后端 `reference_alchabitius` 分支存在且仍为默认
    - 前端下拉项、页面参数传递、AI 导出仍指向同一选项
    - 抽样 case 的 `Arc / promissor / significator / date` 行结构完整，且 `date == SignAscTime.getDateFromPDArc(arc)`
    - 最新 `current120` 校验里 `Asc / MC / North Node` 的 `arc_mae < 0.001°`
    - `reference_alchabitius / horosa_legacy` 两个后端分支都能各自产出非空结果，而且不会误用成同一套 rows
...
- 新一批 current120 基准样本：
  ...
  - `runtime/pd_local/shared_core_geo_current120_v2_exact_rows.csv`
  - `runtime/pd_local/shared_core_geo_current120_v2_exact_summary.json`
  - 当前结果：
    ...
    - 三者都满足 `0.001°` 门槛
- Verification (local):
  - `.runtime/mac/venv/bin/python3 -m py_compile scripts/check_primary_direction_reference_integration.py Horosa-Web/astropy/astrostudy/perpredict.py Horosa-Web/astropy/astrostudy/perchart.py` ✅
  - `.runtime/mac/venv/bin/python3 scripts/check_primary_direction_reference_integration.py` ✅
  - `npm --prefix Horosa-Web/astrostudyui run build:file` ✅
...
### 10:24 - Reference 虚点 current540 对象级 body correction 落地（2026-03-06）
- Scope: 针对 current540 宽域样本里 `Asc / MC / North Node` 主要被 promissor 本体坐标残差拖高的问题，引入只作用于 Reference 虚点行的对象级 body correction。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  - `scripts/train_reference_virtual_body_corrections.py`
  - `scripts/check_primary_direction_reference_integration.py`
  - `Horosa-Web/astropy/astrostudy/models/reference_pd_virtual_body_corr_*.joblib`
  - `runtime/pd_models/virtual_body_corr_current540_holdout/reference_pd_virtual_body_corr_summary_v1.json`
  - `runtime/pd_models/virtual_body_corr_current540_fullfit/reference_pd_virtual_body_corr_summary_v1.json`
- Details:
  - 新增对象级模型训练脚本 `train_reference_virtual_body_corrections.py`，学习“基准 body - local Swiss body”的 `lon / lat` 差值。
...
- Details:
  ...
  - 新增多 case smoke check：
    ...
    - 分别跑 `reference_alchabitius` 与 `horosa_legacy`
    - 检查两条链路都不会抛错、不会空表
  - 新增显示过滤规则镜像检查：
    - `reference_alchabitius` 当前 rows 已全部落在 UI 支持子集内
    - `horosa_legacy` 确认存在 `bound` 行
    - `showPdBounds = 否` 时 legacy 行数会实质下降，证明这个设置不是假开关
  - 新增运行时模型文件存在性检查：
    - `reference_pd_virtual_body_corr_{sun..pluto}_v1.joblib`
  - AI 导出与 AI 导出设置继续检查：
    - `primarydirect` 分段预设
...
### 01:36 - 当前目标口径再确认：run200_login 更接近 geo300，不再让 wide240_v7 主导（2026-03-06）
- Scope: 用当前基准样本重新确认当前复刻目标口径，避免历史异常批次继续误导 `Asc / MC / North Node` 的调优方向。
- Files:
  - `PRIMARY_DIRECTION_ALCHABITIUS_REPLICATION.md`
  - `PROJECT_STRUCTURE.md`
  - `UPGRADE_LOG.md`
- Details:
  - 复跑 `runtime/pd_auto/run200_login` 的虚点 kernel 诊断，产物为：
    - `runtime/pd_local/virtual_kernel_run200_login_v1_summary.json`
  - 当前基准样本结果显示：
    - `Asc arc_mae = 0.0009593129`
    - `MC arc_mae = 0.0001459918`
    - `North Node arc_mae = 0.0001623783`
  - 同时确认当前基准输入的角度本体与本地图盘几乎重合：
    - `Asc mae ≈ 0.0000257°`
    - `MC mae ≈ 0.0000261°`
  - 这说明：
    - 当前目标口径更接近 `geo300`
  ...
  - 进一步对象级排查确认：
    ...
    - 默认 Swiss `apparent` 已是 `Sun..Pluto` 最接近当前目标口径的方案
    - `Moon` 仍是唯一 `TRUEPOS` 比默认 `apparent` 更接近当前目标口径的对象
  - 因此当前策略继续保持：
    - 不按 `wide240_v7` 去改生产角度口径
    - `Moon -> Asc / North Node` 继续保留 `TRUEPOS` 修正
    - 后续若继续压低 `Asc`，优先追 promissor apparent longitude 细节
- Verification (local):
  - `.runtime/mac/venv/bin/python3 scripts/eval_virtual_point_kernels.py --cases-root runtime/pd_auto/run200_login --out-csv runtime/pd_local/virtual_kernel_run200_login_v1_rows.csv --out-json runtime/pd_local/virtual_kernel_run200_login_v1_summary.json` ✅

### 23:11 - 主/界限法页的方法切换改为整表重算，Horosa原方法不再只更新左列（2026-03-05）
- Scope: 修复 Horosa 网站主/界限法页里 `Horosa原方法 / Alchabitius` 切换时，只看到第一列标题变化、右侧三列仍保留旧结果的假切换。
- Files:
  - `Horosa-Web/astrostudyui/src/components/direction/AstroDirectMain.js`
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
  - `PRIMARY_DIRECTION_ALCHABITIUS_REPLICATION.md`
...
- Details:
  ...
  - `Horosa-Web/verify_horosa_local.sh` 现在会串行执行：
    ...
    - `python3 scripts/check_primary_direction_reference_integration.py`
    - `python3 scripts/check_horosa_full_integration.py`
- Verification (local):
  - `./scripts/mac/self_check_horosa.sh` ✅
  - `npm --prefix Horosa-Web/astrostudyui run build:file` ✅
...
- Verification (local):
  - `test -f ALCHABITIUS_PTOLEMY_REVERSE_ENGINEERING_PUBLIC_EDITION.md` ✅
...
- Verification (local):
  - `bash -n Horosa_Local.command Horosa-Web/start_horosa_local.sh` ✅
  - `./scripts/mac/self_check_horosa.sh` ✅
  - `./scripts/mac/self_check_horosa.sh` in packaged folder `~/Desktop/Horosa-Web+App (Mac)` ✅

### 18:35 - 修复 Reference 主限法在浏览器里误走 mundo 分支，固定基准盘重新对齐目标口径（2026-03-06）
- Scope: 修复“主限法页面标题显示 `Alchabitius`，但浏览器实际打出去的是 `pdtype=1` mundo 主限法”的运行态问题。这个问题会让固定基准盘这类样本在用户页面里直接偏离当前目标结果。
- Files:
  - `Horosa-Web/astrostudyui/src/models/astro.js`
  - `Horosa-Web/astropy/websrv/webchartsrv.py`
  - `Horosa-Web/astropy/astrostudy/helper.py`
...
- Details:
  ...
  - 浏览器页再次用固定基准盘复测：
    ...
    - `pageErrors = []`
    - 第一屏关键行重新稳定对回当前目标口径
  - 这说明桌面包用户在浏览器里看到的主限法表格，已经重新和桌面包后端 `Alchabitius` 分支保持一致，并与当前目标口径保持一致。
- Verification (local):
...
### 15:45 - 推运盘新增“十年大运”并补齐专项自检
- Scope: 按用户提供的希占十年大运文档，在 `推运盘 -> 流年法` 下新增独立 `十年大运` 技法页，完成页面、计算、AI 导出与专项自检接线。
- Files:
  ...
  - `scripts/check_horosa_full_integration.py`
- Details:
  - 推运盘右栏在 `流年法` 下新增 `十年大运` 页签，左中右三栏布局参考 `黄道星释`：
    - 左侧继续显示星盘；
    - 中间改为 `L1-L4` 层级树，标题为 `基于X起运`；
    - 右侧顶部从“推运基点选择”改为 `设置`，保留 `AI输出选择`。
  - 十年大运设置项已落地为：
    - `起运主星`：默认 `得时光体（昼日夜月）`，也可手选七曜；
    - `分配次序`：`实际黄道次序 / 迦勒底星序`；
    - `日限体系`：`Valens（精确） / Hephaistio（原表日数）`。
  - 计算模块 `decennials.js` 按文档实现：
    - 七曜基数：土 `30`、木 `12`、火 `15`、日 `19`、金 `8`、水 `20`、月 `25`；
    - 一级总周期固定 `129个月 = 3870天`；
    - `Valens` 路径按比例细分到分钟；
    - `Hephaistio` 路径改为采用文档原表日数，不再用“整数近似保总和”替代；
    - `L4` 继续细分到时主星，并明确显示 `HH:MM`。
  - 中间层级树已按用户要求改用行星符号，不再沿用星座符号；`L1/L2/L3` 右侧时间保留现有日期样式，`L4` 才显示到 `YYYY-MM-DD HH:mm`。
  - AI 导出链路已接入 `decennials`：
    - 新增 `推运盘-十年大运` 技法键；
    - 快照分区为 `起盘信息 / 星盘信息 / 十年大运设置 / 基于X起运`；
    - `基于...起运` 分段标题已加入标准化兼容，避免导出设置过滤失效。
  - 浏览器/静态巡检脚本已补入 `十年大运` 页签识别：
    - `browser_horosa_master_check.py`
    - `check_horosa_full_integration.py`
  - 十年大运专项测试已补齐，覆盖：
    - 昼夜盘 `得时光体` 起运；
    - `实际黄道次序 / 迦勒底星序`；
    - `Valens / Hephaistio` 两套日限；
    - 文档原表值校验；
    - `L1-L4` 时长一致性与 `L4 HH:MM` 展示格式。
- Verification (local):
...
- Details:
  ...
  - 桌面打包版已重新和主仓库对齐，并用广德盘浏览器实测确认：
    ...
    - `主限法盘` 在表格时间、任意时间、`Horosa原方法 <-> external-ref-Alchabitius` 切换下都能同步更新外圈时间与外圈位置。
- Verification (local):
  - `./.runtime/mac/venv/bin/python3 scripts/check_primary_direction_external-ref_integration.py` ✅
...
- Scope: 修复整站自检中暴露出的两个运行态问题：
  - `/chart` 在 `horosa_legacy` 分支下仍可能返回 `external-ref_alchabitius` 的 `params / rows`；
...
- Files:
  ...
  - `README.md`
  - `PROJECT_STRUCTURE.md`
  - `主限法推演/PRIMARY_DIRECTION_external-ref_ALCHABITIUS_REPLICATION.md`
- Details:
  ...
  - 这样做以后：
    - `/chart` 与 `/predict/pd` 在 `external-ref_alchabitius`、`horosa_legacy` 两个分支上的 rows 数量和行内容重新一致；
...
- Verification bundle:
  - `~/Desktop/Horosa/scripts/mac/self_check_horosa.sh`
  - `~/Desktop/Horosa/Horosa-Web/astrostudyui/scripts/verifyPrimaryDirectionRuntime.js`
  - `~/Desktop/Horosa/Horosa-Web/astrostudyui/scripts/verifyHorosaPerformanceRuntime.js`
  - `~/Desktop/Horosa/Horosa-Web/astrostudyui/scripts/verifyHorosaRuntimeFull.js`
  - `~/Desktop/Horosa/scripts/check_primary_direction_external-ref_integration.py`
  - `~/Desktop/Horosa/scripts/check_horosa_full_integration.py`
  - `~/Desktop/Horosa/scripts/browser_horosa_master_check.py`
  - `~/Desktop/Horosa/scripts/browser_primary_direction_chart_guangde_check.py`
  - `~/Desktop/Horosa/scripts/check_local_ephemeris_precision.py --json`
- Result:
...
- Verification:
  ...
  - 浏览器级宗师巡检：
    - `HOROSA_WEB_PORT=8000 HOROSA_SERVER_PORT=9999 HOROSA_SERVER_ROOT=http://127.0.0.1:9999 ~/miniconda3/bin/python scripts/browser_horosa_master_check.py` ✅
    - `runtime/browser_horosa_master_check.json` 状态 `ok`
  - 顶部菜单/管理专项：
    - `HOROSA_WEB_PORT=8000 HOROSA_SERVER_PORT=9999 HOROSA_SERVER_ROOT=http://127.0.0.1:9999 ~/miniconda3/bin/python scripts/browser_horosa_toolbar_management_check.py` ✅
    - `runtime/browser_horosa_toolbar_management_check.json` 状态 `ok`
    - 核验通过项包括：主题切换、AI 导出/设置、命盘管理、事盘管理、批注、小工具、关系盘五种技术、星盘组件/行星/相位选择器
  - 最终排版总检：
    - `HOROSA_WEB_PORT=8000 HOROSA_SERVER_PORT=9999 HOROSA_SERVER_ROOT=http://127.0.0.1:9999 ~/miniconda3/bin/python scripts/browser_horosa_final_layout_check.py` ✅
    - `runtime/final_layout_master_check.json` 状态 `ok`

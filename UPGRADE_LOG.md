...
### 19:40 - 主限法双内核落地、Core-Alchabitius 文档化、UI 同步修复（2026-03-05）
- Scope: 把主限法做成 `Horosa原方法 / Core-Alchabitius` 双内核；同步页面、设置、AI 导出；并把 Core 复刻算法写成可独立照抄的本地文档。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
...
- Details:
  - 后端主限法双分流：
    ...
    - `core_alchabitius`：使用当前 Core 对齐内核：
  ...
  - 文档落地：
    - 新增 `PRIMARY_DIRECTION_CORE_ALCHABITIUS_REPLICATION.md`，完整写明：
      - 行结构
      ...
      - 日期换算
      - 复现脚本与验证指标
    - 新增 `PRIMARY_DIRECTION_CORE_ALCHABITIUS_MATH_FLOW.md`，单独给出：
      - 纯数学定义
      - `ra / raZ` 坐标层
...
- Verification (local):
  ...
  - `npm --prefix Horosa-Web/astrostudyui run build:file` ✅
  - `runtime/pd_reverse/selfcheck_core60_summary.json`：
    - `rows_matched=20196`
  ...
  - `runtime/pd_reverse/local_backend_vs_core_rows_all300_after_displayfilter_summary.json`：
    - `rows_matched=102032`
    - `arc_mae=0.0006788783645654379`
    - `date_mae_days=0.37174043900477144`

### 20:18 - Core 主限法分支移除映点/反映点（2026-03-05）
- Scope: 按页面需求，`Core-Alchabitius` 方法在“计算 + 显示”时都不再包含所有星和虚点的映点 / 反映点（`A_ / C_`）。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
  - `PRIMARY_DIRECTION_CORE_ALCHABITIUS_REPLICATION.md`
...
- Details:
  - 后端：
    - `getPrimaryDirectionByZCoreKernel()` 的 promissor 集从：
      - `N(SIG_OBJECTS, aspects) + TERMS + A + C`
      改为：
      - `N(SIG_OBJECTS, aspects) + TERMS`
    - 即 Core 分支不再计算映点 / 反映点行。
  - 前端：
    - `AstroPrimaryDirection.convertToDataSource()` 对 `core_alchabitius` 再加一道显示保险：
      - 过滤所有 `A_` / `C_` 开头的 promittor / significator 行；
    - 这样即使旧 `chartObj` 尚未重算，页面也不会继续显示映点 / 反映点。
  - 文档：
    - 两份主限法复刻文档同步改为：
      - Core 分支 Promissors 仅包含 `N(...) + TERMS`
      - 不再声明 `A / C` 参与该分支。
- Verification (local):
...
- Scope: 继续收敛 `Core-Alchabitius` 的剩余误差；先只落地已经在样本上稳定成立的 `Asc / MC` 专用公式，`Pars Fortuna` 暂不硬写未经稳定验证的猜测。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  - `PRIMARY_DIRECTION_CORE_ALCHABITIUS_REPLICATION.md`
  - `PRIMARY_DIRECTION_CORE_ALCHABITIUS_MATH_FLOW.md`
  ...
  - `UPGRADE_LOG.md`
- Details:
  - `Core-Alchabitius` 不再把 `Asc / MC` 当普通应星统一走 `sig.ra - prom.raZ`。
  - 新增 `Asc` 专用分支：
...
### 22:58 - Core 主限法收口为 shared core，并改用 true node + 日期黄赤交角（2026-03-05）
- Scope: 按用户要求，Core 主限法现在只保留 Core 网站选项需要的 shared core；取消 `界 / 紫气 / 暗月 / 其它 unsupported virtual points`，并继续收敛 `North Node / Asc / MC` 与 Core 的误差。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  - `Horosa-Web/astropy/astrostudy/signasctime.py`
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
...
- Details:
  ...
  - Core kernel 不再计算：
    ...
    - `South Node`
    - 其它 Core 不支持的扩展虚点
  - Node 口径切换为 `swisseph.TRUE_NODE`，并在 Core kernel 内重建 node 派生点。
  - `SignAscTime` 现支持 `float hour` 输入，验证时可直接使用 `sourceJD` 还原出的精确 UTC 浮点小时。
  - Core kernel 的黄赤转换不再使用 flatlib 固定 `23.44`：
    - 普通对象与 `MC` 改为日期 `mean obliquity`
    - `Asc` 的 zero-lat OA 分支改为日期 `true obliquity`
  - 前端 Core 选项下继续做 unsupported rows 过滤，因此这轮更新只同步到 Horosa 网站里的 Core 计算选项，不影响 `horosa_legacy`。
- Verification (local):
...
### 00:18 - Core-Alchabitius 虚点分支补充 Moon TRUEPOS 试验与稳定修正（2026-03-06）
...
- Details:
  ...
  - 诊断结果表明：当前虚点残差的主体并不在主限弧公式本身，而在 promissor 本体坐标与 Core `chartSubmit` 的细小差异，尤其是 `Moon`。
  - 在 `perpredict.py` 中新增 `_rebuildCoreTruePosMoonPoint()`。
  ...
  - 同时保留实验上界结论：
    - 若用 Core `chartSubmit` 的 promissor 本体位置替代本地位置，`Asc / MC / North Node` 还能明显继续下降；
    - 这说明后续若要逼近 `0.0001°` 量级，重点应放在 Core 本体星历口径，而不是继续大改虚点公式。
- Verification (local):
...
### 09:45 - 主限法 Core 选项接入链路自检补齐，时间主钥切换 remount 修正（2026-03-06）
- Scope: 确认 Horosa 网站里的 `Core-Alchabitius` 选项已经真实接到当前生产算法；同时修复主限法表格在只切换 `pdTimeKey` 时可能复用旧行缓存的问题，并补一个可重复执行的本地自检入口。
- Files:
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
  - `scripts/check_primary_direction_core_integration.py`
  - `PROJECT_STRUCTURE.md`
...
- Details:
  ...
  - 新增 `scripts/check_primary_direction_core_integration.py`，本地自检四层：
    - 后端 `core_alchabitius` 分支存在且仍为默认
    ...
    - `core_alchabitius / horosa_legacy` 两个后端分支都能各自产出非空结果，而且不会误用成同一套 rows
...
- 新一批 current Core 样本：
  ...
  - 当前结果：
    ...
    - 三者都满足 `0.001°` 门槛
- Verification (local):
  - `.runtime/mac/venv/bin/python3 -m py_compile scripts/check_primary_direction_core_integration.py Horosa-Web/astropy/astrostudy/perpredict.py Horosa-Web/astropy/astrostudy/perchart.py` ✅
  - `.runtime/mac/venv/bin/python3 scripts/check_primary_direction_core_integration.py` ✅
  - `npm --prefix Horosa-Web/astrostudyui run build:file` ✅
...
### 10:24 - Core 虚点 current540 对象级 body correction 落地（2026-03-06）
- Scope: 针对 current540 宽域样本里 `Asc / MC / North Node` 主要被 promissor 本体坐标残差拖高的问题，引入只作用于 Core 虚点行的对象级 body correction。
- Files:
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  - `scripts/train_core_virtual_body_corrections.py`
  - `scripts/check_primary_direction_core_integration.py`
  - `Horosa-Web/astropy/astrostudy/models/core_pd_virtual_body_corr_*.joblib`
  - `runtime/pd_models/virtual_body_corr_current540_holdout/core_pd_virtual_body_corr_summary_v1.json`
  - `runtime/pd_models/virtual_body_corr_current540_fullfit/core_pd_virtual_body_corr_summary_v1.json`
- Details:
  - 新增对象级模型训练脚本 `train_core_virtual_body_corrections.py`，学习 `chartSubmit body - local Swiss body` 的 `lon / lat` 差值。
...
- Details:
  ...
  - 新增多 case smoke check：
    ...
    - 分别跑 `core_alchabitius` 与 `horosa_legacy`
    - 检查两条链路都不会抛错、不会空表
  - 新增显示过滤规则镜像检查：
    - `core_alchabitius` 当前 rows 已全部落在 UI 支持子集内
    - `horosa_legacy` 确认存在 `bound` 行
    - `showPdBounds = 否` 时 legacy 行数会实质下降，证明这个设置不是假开关
  - 新增运行时模型文件存在性检查：
    - `core_pd_virtual_body_corr_{sun..pluto}_v1.joblib`
  - AI 导出与 AI 导出设置继续检查：
    - `primarydirect` 分段预设
...
### 01:36 - 当前 Core 口径再确认：run200_login 更接近 geo300，不再让 wide240_v7 主导（2026-03-06）
- Scope: 用登录态抓取的当前 Core 样本重新确认当前复刻目标口径，避免历史异常批次继续误导 `Asc / MC / North Node` 的调优方向。
- Files:
  - `PRIMARY_DIRECTION_CORE_ALCHABITIUS_REPLICATION.md`
  - `PROJECT_STRUCTURE.md`
  - `UPGRADE_LOG.md`
- Details:
  ...
  - 同时确认当前 Core `chartSubmit` 的角度本体与本地图盘几乎重合：
    - `Asc mae ≈ 0.0000257°`
    - `MC mae ≈ 0.0000261°`
  - 这说明：
    - 当前 Core 更接近 `geo300`
  ...
  - 进一步对象级排查确认：
    ...
    - `Moon` 仍是唯一 `TRUEPOS` 比默认 `apparent` 更接近 Core 的对象
  - 因此当前策略继续保持：
...
- Scope: 修复 Horosa 网站主/界限法页里 `Horosa原方法 / Core-Alchabitius` 切换时，只看到第一列标题变化、右侧三列仍保留旧结果的假切换。
- Files:
  - `Horosa-Web/astrostudyui/src/components/direction/AstroDirectMain.js`
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
  - `PRIMARY_DIRECTION_CORE_ALCHABITIUS_REPLICATION.md`
...
- Details:
  ...
  - `Horosa-Web/verify_horosa_local.sh` 现在会串行执行：
    ...
    - `python3 scripts/check_primary_direction_core_integration.py`
    - `python3 scripts/check_horosa_full_integration.py`
- Verification (local):
  - `./scripts/mac/self_check_horosa.sh` ✅
  - `npm --prefix Horosa-Web/astrostudyui run build:file` ✅

### 11:48 - Core 主限法总文档扩写为公开发布整理稿（2026-03-06）
...
- Verification (local):
  - `test -f CORE_ALCHABITIUS_PTOLEMY_REVERSE_ENGINEERING_PUBLIC_EDITION.md` ✅
...
### 18:35 - 修复 Core 主限法在浏览器里误走 mundo 分支，广德盘重新对齐 Core（2026-03-06）
- Scope: 修复“主限法页面标题显示 `Core-Alchabitius`，但浏览器实际打出去的是 `pdtype=1` mundo 主限法”的运行态问题。这个问题会让广德盘这类样本在用户页面里直接偏离 Core 当前网页结果。
- Files:
  - `Horosa-Web/astrostudyui/src/models/astro.js`
  - `Horosa-Web/astropy/websrv/webchartsrv.py`
  - `Horosa-Web/astropy/astrostudy/helper.py`
...
- Details:
  ...
  - Python `/chart` 返回的 `params` 现在显式回传：
    - `pdtype`
    - `showPdBounds`
  - 这样主限法页不再把“后端实际用了 mundo”误判成“已同步的 Core zodiaco 结果”。
  ...
  - 广德样本浏览器复测时，Python 日志已确认请求为：
    ...
    - `date = 2006/10/04`
    - `time = 09:58:00`
  - 广德样本浏览器第一页重新回到 Core 同批结果，前几行已恢复为：
...
### 19:12 - 桌面打包版主限法重新对齐主仓库，广德盘浏览器页重新回到 Core 同批结果（2026-03-06）
- Scope: 修复桌面打包版 `Horosa-Web+App (Mac)` 在冷启动/瘦身后，广德盘主限法第一页重新偏离 Core 的问题。核心不是 Python 主限法内核，而是桌面包 Java `/chart` 还在命中旧缓存/旧编译产物。
- Files:
  - `Horosa-Web/astropy/astrostudy/helper.py`
  - `Horosa-Web/astropy/websrv/webchartsrv.py`
  - `Horosa-Web/astrostudyui/src/components/astro/AstroPrimaryDirection.js`
...
- Details:
  ...
  - 浏览器页再次用广德盘 `2006-10-04 09:58 / 30n53 / 119e25 / guangde` 复测：
    ...
    - 前几行重新稳定为：
      ...
      - `0度57分 / 2007-09-14 13:37:20`
      - `1度33分 / 2008-04-21 15:49:15`
  - 这说明桌面包用户在浏览器里看到的主限法表格，已经重新和桌面包后端 `Core-Alchabitius` 分支保持一致，并与 Core 当前网页保持“所差无几”。
- Verification (local):

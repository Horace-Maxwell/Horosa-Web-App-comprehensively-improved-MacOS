...
这是桌面初始化页、诊断页、设置页对应的前端资源。当前 `index.html` / `style.css` / `app.js` 是 2.1.0 启动控制台：首屏展示 runtime、服务、日志与恢复动作，并配合桌面壳保存上次窗口大小。
...
旧的 `main-workspace.png` 与 `sanshi-workspace.png` 示例图已移除，README 三语都指向这组三张当前桌面截图。

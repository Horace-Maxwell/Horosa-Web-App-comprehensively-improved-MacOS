...
- 日期换算：
  - `Horosa-Web/astropy/astrostudy/signasctime.py`
    - `getJDFromPDArc()`
      ...
      - converse 使用 `abs(arc)`；
    - `getDateFromPDArc()`
      - 使用 UTC 风格输出字符串，贴近 Core `dirDate`。
...
## 103) Core-Alchabitius shared-core 对齐现状（2026-03-05 晚）
...
- 仅影响 Core 选项：
  ...
  - `Horosa-Web/astrostudyui/src/models/astro.js`
  - `Horosa-Web/astrostudyui/src/utils/aiExport.js`
- 当前 Core kernel 的 shared-core 对象集：
...
- 当前关键数学口径：
  ...
  - `Asc` 另有 chart-level `asc_case_bias(chart)` 修正，只对 `Core-Alchabitius` 的 `Asc` 行生效
...
- `asc_case_bias(chart)` 资源与训练入口：
  - 模型文件：`Horosa-Web/astropy/astrostudy/models/core_pd_asc_case_corr_et_v1.joblib`
  - 元数据：`Horosa-Web/astropy/astrostudy/models/core_pd_asc_case_corr_et_v1.json`
  - 训练脚本：`scripts/train_core_asc_case_correction.py`
- 这层修正只作用于 `Asc`：
  - 不改 `MC`
  - 不改 `North Node`
...
- 新增运行时模型：
  - `Horosa-Web/astropy/astrostudy/models/core_pd_virtual_body_corr_sun_v1.joblib`
  - `Horosa-Web/astropy/astrostudy/models/core_pd_virtual_body_corr_moon_v1.joblib`
  - `Horosa-Web/astropy/astrostudy/models/core_pd_virtual_body_corr_mercury_v1.joblib`
  - `Horosa-Web/astropy/astrostudy/models/core_pd_virtual_body_corr_venus_v1.joblib`
  - `Horosa-Web/astropy/astrostudy/models/core_pd_virtual_body_corr_mars_v1.joblib`
  - `Horosa-Web/astropy/astrostudy/models/core_pd_virtual_body_corr_jupiter_v1.joblib`
  - `Horosa-Web/astropy/astrostudy/models/core_pd_virtual_body_corr_saturn_v1.joblib`
  - `Horosa-Web/astropy/astrostudy/models/core_pd_virtual_body_corr_uranus_v1.joblib`
  - `Horosa-Web/astropy/astrostudy/models/core_pd_virtual_body_corr_neptune_v1.joblib`
  - `Horosa-Web/astropy/astrostudy/models/core_pd_virtual_body_corr_pluto_v1.joblib`
- 运行时接入点：
  - `Horosa-Web/astropy/astrostudy/perpredict.py`
  - 新增：
    - `CORE_PD_VIRTUAL_BODY_CORR_MODELS`
    - `_coreLoadVirtualBodyCorrectionModel()`
    - `_coreVirtualBodyCorrectionFeatures()`
    - `_applyCorePromissorBodyModelCorrection()`
- 当前用途：
  - 只作用于 `Core-Alchabitius` 的虚点 significator 行：
    - `Asc`
    - `MC`
    - `North Node`
...
- 作用：
  ...
  - 因此 Horosa 网站里切换 `Core-Alchabitius / Horosa原方法` 时，整站真正换的是后端算法，不是只改前端显示或 AI 导出标签。
- 新增运行时主限法验收脚本：
...
### 103.7) Core-Alchabitius 主限法生产实现记录（2026-03-06）

- 实现说明文档：
  - `PRIMARY_DIRECTION_CORE_ALCHABITIUS_REPLICATION.md`
  - 用途：
    - 记录 Horosa 当前生产版 `Core-Alchabitius` 的完整工程实现
    - 明确区分：
      - 主公式
      - True Node 重建
      - 动态 obliquity
...
- 生产模型目录：
  ...
  - 当前包含：
    - `core_pd_virtual_body_corr_*.joblib`
    - `core_pd_asc_case_corr_et_v1.joblib`
    - `core_pd_asc_case_corr_et_v1.json`
- 说明：
  - 要在别人的 Mac 上“原模原样”复刻当前 Horosa 的 Core 近似主限法，不能只复制 `perpredict.py`；
  - 还必须同时带上：
    - `models/` 目录
    - 前端主限法接线
    - Java 服务层透传
  ...
  - 2026-03-06 下午又补了一层运行态同步要求：
    ...
    - `verifyPrimaryDirectionRuntime.js` / `check_primary_direction_core_integration.py` 必须断言表格直接显示后端 `pd[0]/pd[1]/pd[2]/pd[4]`
...
- 根目录新目录：
  - `WINDOWS_CODEX_CORE_PD_REPRO_KIT/`
- 目标：
  - 让 Windows 上的 Codex 不需要重新研究算法，只依赖这个文件夹就能把当前 Mac 生产版 `Core-Alchabitius` 主限法准确搬到 Windows 仓库。
- 包内结构：
  - `README_FIRST.md`
    - 给人看的超详细复现说明
...
### 103.9) Core 主限法完整逆向推理总文档（2026-03-06）
...
- 配套关系：
  - 它是顶层“全过程总文档”
  - `PRIMARY_DIRECTION_CORE_ALCHABITIUS_REPLICATION.md` 更偏生产实现说明
  - `PRIMARY_DIRECTION_CORE_ALCHABITIUS_MATH_FLOW.md` 更偏数学骨架与流程图
...
### 103.11) Core 主限法公开整理稿（2026-03-06）
...
- 与其它文档的关系：
  - `CORE_ALCHABITIUS_PTOLEMY_REVERSE_ENGINEERING_FULL_PROCESS.md` 保留原始全过程痕迹，更适合内部研究和追索
  - `CORE_ALCHABITIUS_PTOLEMY_REVERSE_ENGINEERING_PUBLIC_EDITION.md` 更适合对外公开发布
  - `PRIMARY_DIRECTION_CORE_ALCHABITIUS_REPLICATION.md` 仍是最贴近生产代码的实现说明

### 103.12) Core 主限法浏览器一致性修复（2026-03-06）
